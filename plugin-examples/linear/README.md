...
```bash
byspace plugin add /absolute/path/to/byspace/plugin-examples/linear
byspace plugin ls linear
```

Use `byspace plugin reload linear` after source edits. Changing the API key requires restarting the
