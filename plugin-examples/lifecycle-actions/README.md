...
```bash
byspace plugin install /absolute/path/to/plugin-examples/lifecycle-actions
byspace plugin install /absolute/path/to/plugin-examples/lifecycle-logger
byspace plugin logs lifecycle-logger
```

Outside the BySpace repository, `server/inspect.ts` needs `@getpaseo/protocol` installed for its
