...
This example adds **Catppuccin Mocha** to Settings → Appearance. BySpace ships Catppuccin as a
...
Register it in `$BYSPACE_HOME/config.json`:
...
```json
{
  ...
  "plugins": {
    "catppuccin": {
      "source": "directory",
      "path": "/absolute/path/to/byspace/plugin-examples/catppuccin"
    }
  }
}
```

Then run `byspace reload` and pick **Catppuccin Mocha** in Settings → Appearance.

The colors come straight from the [Catppuccin Mocha](https://catppuccin.com/palette/) palette:
`base`, `text`, `surface0`, `surface1`, `mauve`, `subtext0`, and `overlay0`. BySpace expands them
