...
describe("shared tool-call display mapping", () => {
  ...
  it("builds display model for worktree setup detail", () => {
    const display = buildToolCallDisplayModel({
      ...
      detail: {
        type: "worktree_setup",
        worktreePath: "/tmp/repo/.byspace/worktrees/repo/branch",
        ...
        commands: [
          {
            index: 1,
            command: "npm install",
            cwd: "/tmp/repo/.byspace/worktrees/repo/branch",
            log: "==> [1/1] Running: npm install\n",
            status: "running",
            exitCode: null,
          },
  ...
  it("humanizes BySpace MCP tool names (Claude Code format)", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__byspace__create_agent",
      status: "running",
      error: null,
  ...
  it("humanizes BySpace MCP tool names (Codex format)", () => {
    const display = buildToolCallDisplayModel({
      name: "byspace.create_agent",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
  ...
  it("humanizes list_agents BySpace tool", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__byspace__list_agents",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
