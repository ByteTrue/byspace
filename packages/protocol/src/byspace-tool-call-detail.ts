import { getBySpaceToolLeafName } from "./tool-name-normalization.js";

export interface BySpaceToolDetailField {
  label: string;
  value: string;
}

export type BySpaceToolDetailSection =
  | {
      kind: "prose";
      title: string;
      text: string;
...
export function buildBySpaceToolDetailSections(
  toolName: string,
  input: unknown,
  output: unknown,
): BySpaceToolDetailSection[] | null {
  const leafName = getBySpaceToolLeafName(toolName);
  if (!leafName) return null;

  const spec = TOOL_SPECS[leafName] ?? {};
  const sections: BySpaceToolDetailSection[] = [];
