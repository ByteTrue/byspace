...
describe("checkout PR schemas", () => {
  test("keeps missing forge identity absent in old daemon payloads", () => {
    ...
    expect(parsed.forge).toBeUndefined();
  });
  ...
  test("accepts unknown future forge identities", () => {
    const parsed = CheckoutPrStatusSchema.parse({
      forge: "someforge",
      url: "https://someforge.example/ByteTrue/byspace/pulls/42",
      title: "Ship it",
      state: "open",
      baseRefName: "main",
      headRefName: "feature/ship-it",
  ...
  test("preserves a github forgeSpecific envelope", () => {
    const parsed = CheckoutPrStatusSchema.parse({
      number: 7,
      url: "https://github.com/ByteTrue/byspace/pull/7",
      title: "Ship it",
      state: "open",
      baseRefName: "main",
      headRefName: "feature/ship-it",
  ...
  test("accepts a GitLab pipeline through the neutral check-details request", () => {
    expect(
      CheckoutForgeGetCheckDetailsRequestSchema.parse({
    ...
    const details = CheckoutForgeGetCheckDetailsResponseSchema.parse({
  ...
  test("accepts optional forge server_info feature flags", () => {
    expect(
      ServerInfoStatusPayloadSchema.parse({
        status: "server_info",
        serverId: "srv_test",
