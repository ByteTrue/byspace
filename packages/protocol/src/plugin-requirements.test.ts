...
describe.each(["daemon", "app"] as const)("plugin requirements on %s", (runtime) => {
  ...
  ])("accepts %s on %s", (byspace, version) => {
    expect(() =>
      assertPluginCompatibility({ id: "test", requirements: { byspace }, version, runtime }),
    ).not.toThrow();
  });
  ...
  it.each([
    ...
    ["^0.8.0", "0.9.0"],
    ["<0.8.0", "0.8.0-beta.1"],
  ])("rejects %s on %s", (byspace, version) => {
    expect(() =>
      assertPluginCompatibility({ id: "test", requirements: { byspace }, version, runtime }),
  ...
  it.each(["", "   ", "latest", ">=potato", "0.8.0 nonsense"])(
    ...
    (byspace) => {
      expect(() => validatePluginRequirements({ byspace })).toThrow("Invalid requirements.byspace");
    },
  );
  ...
  it.each([null, "unknown"])("fails closed when the runtime version is %s", (version) => {
    expect(() =>
      assertPluginCompatibility({
        id: "test",
        requirements: { byspace: "*" },
        version,
        runtime,
      }),
    ).toThrow(`BySpace ${runtime} version is unknown`);
  });
});
