...
import { buildBySpaceToolDetailSections } from "./byspace-tool-call-detail.js";

describe("BySpace tool-call detail presentation", () => {
  it.each(["mcp__byspace__create_agent", "byspace.create_agent", "byspace_remote.create_agent"])(
  ...
  it("formats schedule cadence and nested settings without JSON syntax", () => {
    const sections = buildBySpaceToolDetailSections(
      "mcp__byspace__create_schedule",
      {
  ...
  it("unwraps MCP result envelopes instead of exposing JSON-encoded text", () => {
    expect(
      buildBySpaceToolDetailSections(
        "mcp__byspace__send_agent_prompt",
        { prompt: "Say hello back." },
        {
          meta: null,
          content: [
  ...
  it("uses readable fallback fields for newly added BySpace tools", () => {
    expect(
      buildBySpaceToolDetailSections(
        "mcp__byspace__future_tool",
  ...
  it("leaves non-BySpace tools alone", () => {
    expect(buildBySpaceToolDetailSections("mcp__github__create_issue", {}, {})).toBeNull();
  });
});
