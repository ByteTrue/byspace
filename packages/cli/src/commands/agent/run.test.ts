...
describe("runRunCommand option validation", () => {
  ...
  it("rejects --worktree combined with an ambient BYSPACE_WORKSPACE_ID", async () => {
    process.env.BYSPACE_WORKSPACE_ID = "ws-ambient";
    await expectInvalidOptions(
      { worktree: "feat" },
      /--worktree cannot be combined with an ambient BYSPACE_WORKSPACE_ID/,
    );
  });

  it("allows a bare --worktree through validation when no workspace is selected", async () => {
    // A bare --worktree with no --workspace and no ambient BYSPACE_WORKSPACE_ID
