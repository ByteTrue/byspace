...
function createFakeDaemonClient(
  overrides: Partial<
    Pick<DaemonClient, "getBySpaceWorktreeList" | "archiveBySpaceWorktree" | "close">
  > = {},
): DaemonClient {
  return {
    getBySpaceWorktreeList: async () => ({
      worktrees: [],
      error: null,
      requestId: "req-list",
    }),
    archiveBySpaceWorktree: async () => ({
      success: true,
      removedAgents: [],
      error: null,
...
describe("runArchiveCommand", () => {
  it("sends scope worktree when archiving by worktree path", async () => {
    const worktreePath = "/tmp/byspace-home/worktrees/repo/feature";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveBySpaceWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
      ...
      archiveBySpaceWorktree: async (input) => {
  ...
  it("archives by matching branch name when no directory name matches", async () => {
    const worktreePath = "/tmp/byspace-home/worktrees/repo/feature-branch";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveBySpaceWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
