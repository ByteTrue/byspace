...
import { DEFAULT_RELAY_ENDPOINT } from "@bytetrue/protocol/daemon-endpoints";
import { loadConfig, resolveBySpaceHome, spawnProcess } from "@bytetrue/server";
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@bytetrue/server") return null;
...
export function resolveDaemonRunnerEntry(): string {
  ...
  throw new Error("Unable to resolve @bytetrue/server package root for daemon runner");
}
