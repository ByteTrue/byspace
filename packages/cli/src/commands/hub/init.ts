...
export async function runHubGuidedSetup(
  ...
  if (deploy) {
    ...
    reportMessage(environment, "Skipped deployment. Run `byspace hub deploy` when ready.");
  }
  ...
  if (connection.kind === "connected") {
    if (permissions.includes("hub.execute") && !status.permissions.includes("hub.execute")) {
      throw new HubCommandError(
        ...
        "This daemon is connected to Hub but cannot run Hub automations. Run `byspace hub permissions grant hub.execute`, then run Hub init again.",
      );
    }
      ...
      while (true) {
        ...
        if (state.kind === "ready") {
          ...
          throw new HubCommandError(
            ...
            "No agent runtime with an execution mode is available from this daemon. Configure one, then run byspace hub init again.",
          );
        }
