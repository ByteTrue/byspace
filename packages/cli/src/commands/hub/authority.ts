...
export function resolveHubOrigin(input: ResolveHubInput): string {
  const configuredOrigin =
    input.options.origin ?? input.env.BYSPACE_HUB_URL ?? input.env.PASEO_HUB_URL;
...
export function resolveHubCredential(input: ResolveHubInput & { origin: string }): string {
  const explicitCredential =
    input.options.apiKey ?? input.env.BYSPACE_HUB_API_KEY ?? input.env.PASEO_HUB_API_KEY;
  ...
  throw new HubCommandError(
    ...
    `No stored Hub login matches ${input.origin}. Run \`byspace hub login ${input.origin}\`, pass --api-key <secret>, or set BYSPACE_HUB_API_KEY.`,
  );
}
