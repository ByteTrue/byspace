...
try {
  {
    ...
    const created = await ctx.byspace([
      "chat",
      "create",
      "coord-room",
      "--purpose",
...

  {
    ...
    const posted = await ctx.byspace(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { BYSPACE_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.byspace(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.byspace(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.byspace(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.byspace(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.byspace(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.byspace(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.byspaceHome, { recursive: true, force: true });
