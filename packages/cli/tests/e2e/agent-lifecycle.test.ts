 ...
 * 2. Create agent with `byspace run "say hello" --provider claude`
 * 3. List agents with `byspace ls`
 * 4. Wait for agent with `byspace wait <id>`
 * 5. Inspect agent with `byspace inspect <id>`
 * 6. Stop agent with `byspace stop <id>` and verify it remains inspectable
 * 7. Delete agent with `byspace delete <id>`
 * 8. Cleanup: stop daemon, remove temp dirs
 *
 * CRITICAL RULES:
 * - NEVER use port 6777 (user's running daemon)
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`BYSPACE_HOME: ${ctx.byspaceHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.byspace(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.byspace(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.byspace(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.byspace(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.byspace(["stop", agentId]);
  ...
  const inspectResult = await ctx.byspace(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.byspace(["delete", agentId]);
