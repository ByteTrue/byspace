...
import { buildRelayWebSocketUrl } from "@byspace/protocol/daemon-endpoints";
import { parseConnectionOfferFromUrl } from "@byspace/protocol/connection-offer";
import { generateLocalPairingOffer } from "@byspace/server";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        BYSPACE_RELAY_ENABLED: "true",
        BYSPACE_RELAY_ENDPOINT: relayEndpoint,
        BYSPACE_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });
