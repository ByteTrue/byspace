...
const byspaceHome = await mkdtemp(join(tmpdir(), "byspace-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${byspaceHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
...

  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${byspaceHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
