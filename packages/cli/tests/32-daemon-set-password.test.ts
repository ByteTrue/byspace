...
import { isBearerTokenValid } from "@bytetrue/byspace-server";
import {
...
const byspaceHome = join(root, ".byspace");
...
try {
  {
      ...
      `${JSON.stringify(
        {
          ...
          app: { baseUrl: "https://byspace.pages.dev" },
        },
        null,
        2,
      )}\n`,
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: byspaceHome });
    const config = JSON.parse(await readFile(join(byspaceHome, "config.json"), "utf-8"));

    assert.strictEqual(result.configPath, join(byspaceHome, "config.json"));
