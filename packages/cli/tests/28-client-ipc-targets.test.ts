...

{
  ...
  const target = resolveDaemonTarget("/tmp/byspace.sock");
  assert.deepStrictEqual(target, {
    type: "ipc",
    url: "ws+unix:///tmp/byspace.sock:/ws",
    socketPath: "/tmp/byspace.sock",
...
{
  console.log("Test 7b: legacy BySpace home, listen, and host variables are ignored");
  ...
  try {
    ...
    assert.deepStrictEqual(
      resolveDefaultDaemonHosts({
        BYSPACE_HOME: byspaceHome,
        BYSPACE_LISTEN: "127.0.0.1:6767",
      }),
      ["localhost:6777"],
    );
    process.env.BYSPACE_HOME = byspaceHome;
    delete process.env.BYSPACE_HOST;
    process.env.BYSPACE_HOST = "127.0.0.1:6767";
  ...
  console.log("✓ legacy BySpace home, listen, and host variables are ignored\n");
}
