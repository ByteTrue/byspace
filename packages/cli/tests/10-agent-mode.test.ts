...
try {
  ...
  // Test 2: agent mode requires id argument
  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${paseoHome} npx paseo agent mode`.nothrow();
  ...
  // Test 3: agent mode handles daemon not running
  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${paseoHome} npx paseo agent mode abc123 bypass`.nothrow();
  ...
  // Test 4: agent mode --list flag is accepted
  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${paseoHome} npx paseo agent mode --list abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: agent mode with ID and --host flag is accepted
  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${paseoHome} npx paseo agent mode abc123 plan --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with agent mode
  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${paseoHome} npx paseo -q agent mode abc123 bypass`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: agent mode requires mode argument when not using --list
  {
    ...
    const result =
      await $`BYSPACE_HOST=localhost:${port} BYSPACE_HOME=${paseoHome} npx paseo agent mode abc123`.nothrow();
