...
$env:EXPO_PUBLIC_BYSPACE_DEV_BUILD_LABEL = (git -C $RootDir branch --show-current).Trim()

$env:BYSPACE_DEV_ROOT = $RootDir
$env:BYSPACE_DEV_RUNTIME_FALLBACK_ROOT = $RootDir
$DevRuntime = node "$ScriptDir\dev-runtime.mjs" | ConvertFrom-Json
$env:BYSPACE_ELECTRON_FLAGS = $DevRuntime.electronFlags
$env:BYSPACE_ELECTRON_USER_DATA_DIR = $DevRuntime.userDataDir
Remove-Item Env:\BYSPACE_DEV_RUNTIME_FALLBACK_ROOT -ErrorAction SilentlyContinue
...
# BYSPACE_HOME defaults to a script-managed dev home. If you override it (to point
...
if (-not $env:BYSPACE_HOME) {
    $env:BYSPACE_HOME = "$DevStateDir\byspace-home"
    $BySpaceHomeManaged = $true
} else {
    $BySpaceHomeManaged = $false
}
New-Item -ItemType Directory -Force -Path $env:BYSPACE_HOME, $env:BYSPACE_ELECTRON_USER_DATA_DIR | Out-Null

$DevDaemonPort = if ($env:BYSPACE_DEV_DAEMON_PORT) { $env:BYSPACE_DEV_DAEMON_PORT } else { "6788" }
if (-not $env:BYSPACE_LISTEN) { $env:BYSPACE_LISTEN = "127.0.0.1:$DevDaemonPort" }
...
# (it does NOT honor the BYSPACE_LISTEN env var) and probing that address. Without
# this it reads the default 6777, finds a production daemon there, and connects
...
# ONLY seed the script-managed home: never rewrite a user-supplied BYSPACE_HOME
# (that could clobber a production config.json with the dev port + wildcard CORS).
if ($BySpaceHomeManaged) {
    $env:TMP_CFG_PATH = "$($env:BYSPACE_HOME)/config.json"
...
"@
    ...
    Write-Host "  (custom BYSPACE_HOME - leaving its config.json untouched)"
}

Write-Host @"
======================================================
  BySpace Desktop Dev (Windows)
======================================================
  Metro:      http://localhost:$($env:EXPO_PORT)
  Daemon:     $($env:BYSPACE_LISTEN) (isolated)
  BYSPACE_HOME: $($env:BYSPACE_HOME)
  userData:   $($env:BYSPACE_ELECTRON_USER_DATA_DIR)
======================================================
"@
...
concurrently `
    ...
    "cd `"$AppDir`" && cross-env BYSPACE_WEB_PLATFORM=electron npx expo start --port $($env:EXPO_PORT)" `
