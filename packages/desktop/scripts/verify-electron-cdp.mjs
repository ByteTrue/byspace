...
const CDP_PORT = process.env.BYSPACE_ELECTRON_REMOTE_DEBUGGING_PORT ?? "9223";
...
const APP_ORIGIN = process.env.ELECTRON_VERIFY_APP_ORIGIN ?? `http://${APP_URL_FRAGMENT}`;
...
async function inspectAppSidebarChrome(page) {
  return page.evaluate(() => {
    ...
    const visibleMenuToggleRects = Array.from(
      document.querySelectorAll('[data-testid="menu-button"]'),
    )
    ...
    return {
      ...
      sidebarHeaderRect: rect('[data-testid="sidebar-pages-header"]'),
      firstSidebarRowRect: rect('[data-testid="sidebar-pages"]'),
      visibleMenuToggleRects,
    };
  });
}
...
async function readBridgeFullscreen(page) {
  return page.evaluate(
    async () =>
      (await window.byspaceDesktop?.window?.getCurrentWindow?.()?.isFullscreen?.()) === true,
  );
}

async function setNativeFullscreen(page, fullscreen) {
  await page.evaluate(async (nextFullscreen) => {
    const win = window.byspaceDesktop?.window?.getCurrentWindow?.();
...
async function inspectFullscreenWindowChrome(page, platform) {
  ...
  try {
    ...
    const details = await page.evaluate(async () => {
      const bridge = window.byspaceDesktop?.window?.getCurrentWindow?.();
...
async function navigateToWelcome(page) {
  ...
  if (!page.url().endsWith("/welcome")) {
    await page.goto(`${APP_ORIGIN}/welcome`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1000);
  }
}
...
async function detectDesktopBridge(page) {
  return page.evaluate(() => {
    const bridge = window.byspaceDesktop;
...
async function navigateToOpenProject(page) {
  await page.evaluate(() => {
    window.location.href = "/open-project";
  });
  const menuToggle = page.locator('[data-testid="menu-button"]').first();
  const sidebarHeader = page.getByTestId("sidebar-pages-header");
  await menuToggle.waitFor({ state: "visible", timeout: 30_000 });
  if (!(await sidebarHeader.isVisible().catch(() => false))) {
    await menuToggle.click();
  }
  await sidebarHeader.waitFor({ state: "visible", timeout: 30_000 });
}
...
async function collectAppSidebarChromeResult(page, platform, results) {
  const expandedDetails = await inspectAppSidebarChrome(page);
  const expandedObstruction =
    platform === "darwin" ? getWindowChromeObstruction(platform, expandedDetails.innerWidth) : null;
  const sidebarRight = expandedDetails.sidebarHeaderRect
    ? expandedDetails.sidebarHeaderRect.left + expandedDetails.sidebarHeaderRect.width
    : null;
  const expandedToggleRect = expandedDetails.visibleMenuToggleRects[0] ?? null;
  const firstRowClearsWindowChrome = Boolean(
    expandedDetails.firstSidebarRowRect &&
    (!expandedObstruction ||
      !rectsIntersect(expandedDetails.firstSidebarRowRect, expandedObstruction)),
  );
  const onlyContentToggleVisible = Boolean(
    expandedDetails.visibleMenuToggleRects.length === 1 &&
    expandedToggleRect &&
    sidebarRight !== null &&
    expandedToggleRect.left >= sidebarRight - 1,
  );
  const expandedScreenshot = await captureScreenshot(page, "02-app-sidebar-chrome.png");

  await page.locator('[data-testid="menu-button"]:visible').first().click();
  await page.locator('[data-testid="sidebar-pages"]').waitFor({ state: "hidden" });
  const collapsedDetails = await inspectAppSidebarChrome(page);
  const collapsedObstruction =
    platform === "darwin"
      ? getWindowChromeObstruction(platform, collapsedDetails.innerWidth)
      : null;
  const collapsedToggleRect = collapsedDetails.visibleMenuToggleRects[0] ?? null;
  const collapsedToggleClearsWindowChrome = Boolean(
    collapsedDetails.visibleMenuToggleRects.length === 1 &&
    collapsedToggleRect &&
    (!collapsedObstruction || !rectsIntersect(collapsedToggleRect, collapsedObstruction)),
  );
  const collapsedScreenshot = await captureScreenshot(page, "03-app-sidebar-collapsed-chrome.png");

  await page.locator('[data-testid="menu-button"]:visible').first().click();
  await page.locator('[data-testid="sidebar-pages"]').waitFor({ state: "visible" });

  results.push({
    check: "app-sidebar-window-chrome",
    pass: Boolean(
      expandedDetails.sidebarHeaderRect &&
      firstRowClearsWindowChrome &&
      onlyContentToggleVisible &&
      collapsedToggleClearsWindowChrome,
    ),
    details: {
      platform,
      expanded: {
      ...
      collapsed: {
        ...
        collapsedToggleClearsWindowChrome,
        ...collapsedDetails,
        screenshot: collapsedScreenshot,
      },
    },
...
async function main() {
  ...
  try {
    ...
    const desktopStatus = await page.evaluate(() =>
      window.byspaceDesktop.invoke("desktop_daemon_status"),
    );
    assert(
    ...
    await navigateToOpenProject(page);
    await collectAppSidebarChromeResult(page, desktopDetection.platform, results);
