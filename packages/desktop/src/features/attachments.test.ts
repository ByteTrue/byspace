...
async function useTempBySpaceHome(): Promise<string> {
  testHome = await mkdtemp(path.join(os.tmpdir(), "byspace-desktop-attachments-"));
  process.env.BYSPACE_HOME = testHome;
  return testHome;
}
...
describe("desktop attachment files", () => {
  ...
  it("accepts dot-prefixed picker extensions for managed copies", async () => {
    const byspaceHome = await useTempBySpaceHome();
    const sourcePath = path.join(byspaceHome, "report.md");
    ...
    expect(result).toEqual({
      path: path.join(byspaceHome, "desktop-attachments", "att_markdown.md"),
      byteSize: 9,
    });
  ...
  it("normalizes legacy bare extensions for managed copies", async () => {
    ...
    expect(result).toEqual({
      path: path.join(byspaceHome, "desktop-attachments", "att_markdown_legacy.md"),
      byteSize: 9,
    });
    await expect(readFile(result.path, "utf8")).resolves.toBe("# Report\n");
  });
