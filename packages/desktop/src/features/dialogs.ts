...
export function registerDialogHandlers(): void {
  ipcMain.handle("byspace:dialog:ask", async (event, message: string, options?: AskOptions) => {
  ...
  ipcMain.handle(
    "byspace:dialog:askWithCheckbox",
  ...
  ipcMain.handle("byspace:dialog:open", async (event, options?: OpenOptions) => {
    const win = BrowserWindow.fromWebContents(event.sender);
