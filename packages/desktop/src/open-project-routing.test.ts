...
describe("open-project-routing", () => {
  it("returns a bare absolute path argument", () => {
    const projectPath = mkdtempSync(path.join(tmpdir(), "byspace-open-project-"));

    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/BySpace.app/Contents/MacOS/BySpace", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
  ...
  it("does not treat flags as project paths", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/BySpace.app/Contents/MacOS/BySpace", "--version", flagLikeDirectory],
        isDefaultApp: false,
      }),
  ...
  it("returns the path from an explicit --open-project flag for backward compatibility", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/BySpace.app/Contents/MacOS/BySpace", "--open-project", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
