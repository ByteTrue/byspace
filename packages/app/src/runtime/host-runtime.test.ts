...
describe("HostRuntimeStore", () => {
  ...
  it("persists the v3 Relay client authentication capability", async () => {
    const store = new HostRuntimeStore({
      deps: {
        createClient: () => new FakeDaemonClient() as unknown as DaemonClient,
        connectToDaemon: async ({ host }) => ({
    ...
    await store.upsertConnectionFromOfferUrl(
      encodeOfferUrl({
        v: 3,
        serverId: "srv_AAAAAAAAAAAA",
        daemonPublicKeyB64: token,
    ...
    const pairedHost = store.getHosts().find((host) => host.serverId === "srv_AAAAAAAAAAAA");
    expect(pairedHost?.connections[0]).toMatchObject({
      type: "relay",
      daemonPublicKeyB64: token,
      clientAuthTokenB64: token,
    });
  ...
  it("serializes an initial pairing offer after registry load migrations", async () => {
    const storedHost = makeHost({ serverId: "srv_stored" });
    const placeholderHost = makeHost({ serverId: "local:bootstrap" });
    const values = new Map<string, string>([
      ["@paseo:daemon-registry", JSON.stringify([storedHost, placeholderHost])],
      ["@paseo:e2e", "1"],
    ]);
    const registryRead = createDeferred<void>();
    const releaseRegistryRead = createDeferred<void>();
    const cleanupWrite = createDeferred<void>();
    const releaseCleanupWrite = createDeferred<void>();
    ...
    const storage: HostRuntimeStorage = {
      ...
      setItem: async (key, value) => {
        if (key === "@paseo:daemon-registry" && registryWrites++ === 0) {
    ...
    const pairing = store.upsertConnectionFromOffer({
      v: 3,
      serverId: "srv_AAAAAAAAAAAA",
      daemonPublicKeyB64: token,
      clientAuthTokenB64: token,
    ...
    expect(values.get("@paseo:daemon-registry")).toContain("srv_stored");
    expect(values.get("@paseo:daemon-registry")).toContain("srv_AAAAAAAAAAAA");
    expect(values.get("@paseo:daemon-registry")).not.toContain("local:bootstrap");
    store.syncHosts([]);
  });
