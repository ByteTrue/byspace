...
function encodeOfferUrl(payload: unknown): string {
  const encoded = Buffer.from(JSON.stringify(payload), "utf8")
    ...
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
  return `https://byspace.pages.dev/#offer=${encoded}`;
}
...
describe("HostRuntimeController", () => {
  it("replaces the active relay client when re-pairing changes the daemon public key", async () => {
    const oldRelay: HostConnection = {
      id: "relay:wss:byspace-relay.bytetrue.workers.dev:443",
      type: "relay",
      relayEndpoint: "byspace-relay.bytetrue.workers.dev:443",
      useTls: true,
  ...
  it("keeps known hosts in connecting when a created client reports idle during connect", async () => {
    ...
    await controller.activateConnection({ connectionId: "direct:lan:6777" });

    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6777");
  ...
  it("keeps browser client lifecycle tied to the active host runtime client", async () => {
    const host = makeHost({ preferredConnectionId: "direct:lan:6777" });
    ...
    await controller.start({
      autoProbe: false,
      initialConnection: {
        connectionId: "direct:lan:6777",
        existingClient: fakeClient as unknown as DaemonClient,
      },
    });

    expect(lifecycle.active).toEqual([{ serverId: "srv_test", connectionId: "direct:lan:6777" }]);
  ...
  it("adopts the first successful probe on startup", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6777": 82,
      "relay:byspace-relay.bytetrue.workers.dev:443": 18,
    };
    const controller = new HostRuntimeController({
      host,
    ...
    expect(snapshot.activeConnectionId).toBe("direct:lan:6777");
  ...
  it("activates the first successful probe without waiting for slower probes", async () => {
    ...
    const controller = new HostRuntimeController({
      ...
      deps: {
        ...
        connectToDaemon: async ({ host: hostProfile, connection }) => {
          const client = makeConnectedProbeClient(connection.id === "direct:lan:6777" ? 12 : 30);
          if (connection.id === "relay:byspace-relay.bytetrue.workers.dev:443") {
    ...
    while (Date.now() < timeoutAt) {
      const snapshot = controller.getSnapshot();
      if (
        snapshot.activeConnectionId === "direct:lan:6777" &&
  ...
  it("ranks the live connection by its heartbeat RTT without pinging it again", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6777": 12,
      "relay:byspace-relay.bytetrue.workers.dev:443": 65,
    };
    const controller = new HostRuntimeController({
      host,
      deps: {
    ...
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6777");
    expect(controller.getSnapshot().connectionStatus).toBe("online");
    expect(controller.getSnapshot().probeByConnectionId.get("direct:lan:6777")).toEqual({
      status: "available",
      latencyMs: 42,
    });
  ...
  it("rejects probes that resolve to a different server id", async () => {
    ...
    expect(controller.getSnapshot().probeByConnectionId.get("direct:localhost:6777")).toEqual({
      status: "unavailable",
      latencyMs: null,
    });
  ...
  it("keeps the live connection when one probe cycle looks slow", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6777": 15,
      "relay:byspace-relay.bytetrue.workers.dev:443": 55,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:byspace-relay.bytetrue.workers.dev:443"] = 42;
  ...
  it("backs off inactive connection probes while a host is online", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6777": 10,
      "relay:byspace-relay.bytetrue.workers.dev:443": 50,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    const initialRelayProbe = controller
      .getSnapshot()
      .probeByConnectionId.get("relay:byspace-relay.bytetrue.workers.dev:443");

    latencies["direct:lan:6777"] = 12;
    latencies["relay:byspace-relay.bytetrue.workers.dev:443"] = 25;
    ...
    expect(snapshot.probeByConnectionId.get("direct:lan:6777")).toEqual({
      status: "available",
      latencyMs: 12,
    });
    expect(
      snapshot.probeByConnectionId.get("relay:byspace-relay.bytetrue.workers.dev:443"),
    ).toEqual(initialRelayProbe);
  });
  ...
  it("switches only after the faster alternative wins consecutive probes", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6777": 15,
      "relay:byspace-relay.bytetrue.workers.dev:443": 60,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["direct:lan:6777"] = 95;
    latencies["relay:byspace-relay.bytetrue.workers.dev:443"] = 30;
    ...
    let switched =
      ...
      "relay:byspace-relay.bytetrue.workers.dev:443";
  ...
  it("does not switch on a transient latency spike", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6777": 15,
      "relay:byspace-relay.bytetrue.workers.dev:443": 80,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["direct:lan:6777"] = 100;
    latencies["relay:byspace-relay.bytetrue.workers.dev:443"] = 20;
    activeClient.heartbeatReportsRtt(100);
    await vi.advanceTimersByTimeAsync(120_000);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:6777");

    latencies["direct:lan:6777"] = 20;
    latencies["relay:byspace-relay.bytetrue.workers.dev:443"] = 90;
  ...
  it("exposes one snapshot with active connection and status from same source", async () => {
    ...
    expect(latest?.activeConnectionId).toBe("direct:lan:6777");
  ...
  it("ignores stale switch failures after a newer connection is already online", async () => {
    ...
    const deps: HostRuntimeControllerDeps = {
      createClient: ({ connection }) => {
        const client = new FakeDaemonClient();
        if (connection.id === "direct:lan:6777") {
    ...
    const switchDirect = controller.activateConnection({ connectionId: "direct:lan:6777" });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
    ...
    const switchRelay = controller.activateConnection({
      connectionId: "relay:byspace-relay.bytetrue.workers.dev:443",
    });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
        snapshot.activeConnectionId === "relay:byspace-relay.bytetrue.workers.dev:443" &&
        snapshot.connectionStatus === "online"
      );
    });
    ...
    const snapshot = controller.getSnapshot();
    expect(snapshot.activeConnectionId).toBe("relay:byspace-relay.bytetrue.workers.dev:443");
    expect(snapshot.connectionStatus).toBe("online");
  ...
  it("coalesces overlapping probe cycles instead of invalidating the in-flight result", async () => {
    ...
    const probeAfterCycle = controller.getSnapshot().probeByConnectionId.get("direct:lan:6777");
...
describe("HostRuntimeStore", () => {
  ...
  it("drains legacy snapshot and buffered running transitions exactly once", async () => {
    const host = makeHost({
      ...
      connections: [{ id: "direct:lan:6778", type: "directTcp", endpoint: "lan:6778" }],
    });
    const fakeClient = new FakeDaemonClient();
    fakeClient.setConnectionState({ status: "connected" });
  ...
  it("records unavailable startup probes when no connection can be established", async () => {
    ...
    while (
      snapshot?.probeByConnectionId.get("direct:lan:6777")?.status !== "unavailable" &&
      Date.now() < timeoutAt
    ) {
      await new Promise((resolve) => setTimeout(resolve, 0));
  ...
  it("probeAndUpsertConnection replaces a matching placeholder host with the real server id", async () => {
    ...
    (
      ...
      makeHost({
        serverId: "local:lan:6777",
        label: "local:lan:6777",
        connections: [connection],
  ...
  it("uses TLS for old pairing URLs that omit relay TLS on port 443", async () => {
    ...
    const oldPairingUrl = encodeOfferUrl({
      ...
      serverId: "srv_offer",
      daemonPublicKeyB64: "pk_test_offer",
      relay: { endpoint: "byspace-relay.bytetrue.workers.dev:443" },
    });
...
describe("HostRuntimeStore initial connection hint bootstrap", () => {
  it("attempts the explicit initial connection hint before default localhost bootstrap", async () => {
    ...
    expect(seenProbes).toContainEqual({ endpoint: "daemon-origin:6777", useTls: true });
    const host = store.getHosts()[0];
    expect(host?.serverId).toBe("srv_hint");
    expect(host?.connections).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ endpoint: "daemon-origin:6777", useTls: true }),
      ]),
    );
