...
describe("FileEditorModel", () => {
  ...
  test("reloads content before adopting a newly precise revision", async () => {
    ...
    expect(model.getSnapshot()).toMatchObject({
      status: "clean",
      content: "new!",
      version: { revision: "precise-revision" },
      observedVersion: { revision: "precise-revision" },
    });
  });
  ...
  test("coalesces edits made while a save is pending", async () => {
    ...
    expect(session.writes).toEqual([
      { content: "saving", expectedModifiedAt: "2026-07-18T00:00:00.000Z" },
      {
        content: "newer edit",
  ...
  test("marks an unavailable clean file as modified because the buffer is now the only copy", () => {
    const { model } = makeModel();
