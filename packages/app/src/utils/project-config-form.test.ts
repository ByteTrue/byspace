...
import { BySpaceConfigRawSchema } from "@bytetrue/protocol/byspace-config-schema";
import type { BySpaceConfigRaw } from "@bytetrue/protocol/messages";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: BySpaceConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = BySpaceConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
