...
describe("test-daemon-connection connectToDaemon", () => {
  ...
  it("allows native direct connections when window.location is unavailable", async () => {
    ...
    expect(probe.createdClients).toHaveLength(1);
  });
  ...
  it("encodes local socket probes for the Electron transport", async () => {
    const { connectToDaemon } = await import("./test-daemon-connection");
    const result = await connectToDaemon(
      {
