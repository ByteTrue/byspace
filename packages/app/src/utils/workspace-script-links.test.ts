...
import type { WorkspaceScriptPayload } from "@bytetrue/protocol/messages";
...
const runningService: WorkspaceScriptPayload = {
  ...
  port: 3000,
  localProxyUrl: "http://web--feature--byspace.localhost:6767",
  publicProxyUrl: null,
  proxyUrl: "http://web--feature--byspace.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("defaults to the memorable BySpace URL locally and keeps direct as a fallback", () => {
    expect(
      ...
      primary: {
        kind: "byspace",
        label: "web--feature--byspace.localhost:6767",
        url: "http://web--feature--byspace.localhost:6767",
      },
      targets: [
        {
          kind: "byspace",
  ...
  it("defaults to an explicitly configured reverse proxy", () => {
    ...
    expect(
      resolveLink(
        { type: "directSocket", endpoint: "/tmp/byspace.sock", display: "socket" },
