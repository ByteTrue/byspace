import type { ToolCallDetail, ToolCallIconName } from "@byspace/protocol/agent-types";
import { isBySpaceToolName } from "@byspace/protocol/tool-name-normalization";

export type ToolCallIcon = ToolCallIconName | "byspace";
...
export function resolveToolCallIconName(toolName: string, detail?: ToolCallDetail): ToolCallIcon {
  ...
  if (isBySpaceToolName(lowerName)) {
    return "byspace";
  }
  if (lowerName === "task") {
    return "bot";
