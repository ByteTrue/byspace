...
function buildAnnotationMarkerScript(markers: readonly BrowserAnnotationMarker[]): string {
  ...
  return `
    (function() {
      var markers = ${payload};
      if (window.__byspaceAnnotationMarkers) { window.__byspaceAnnotationMarkers.update(markers); return true; }
      ...
      window.__byspaceAnnotationMarkers = {
        ...
        destroy: function() {
          ...
          window.__byspaceAnnotationMarkers = null;
        }
      };
      reposition();
      return true;
...
function clearAnnotationMarkers(webview: ElectronWebview): void {
  void executeWebviewJavaScript(
    webview,
    "if(window.__byspaceAnnotationMarkers) window.__byspaceAnnotationMarkers.destroy();",
...
const ThemedWrench = withUnistyles(Wrench, (theme) => ({
  color: theme.colors.foregroundMuted,
}));
const ThemedMousePointerMuted = withUnistyles(MousePointer2, (theme) => ({
  color: theme.colors.foregroundMuted,
}));
const ThemedMousePointerActive = withUnistyles(MousePointer2, (theme) => ({
  color: theme.colors.accent,
}));
const ThemedCameraMuted = withUnistyles(Camera, (theme) => ({
  color: theme.colors.foregroundMuted,
}));
const ThemedCameraActive = withUnistyles(Camera, (theme) => ({
  color: theme.colors.accent,
}));
const ThemedUrlInput = withUnistyles(TextInput, (theme) => ({
  placeholderTextColor: theme.colors.foregroundMuted,
}));
const URL_INPUT_OUTLINE_STYLE = { outlineStyle: "none" } as object;
...
export function BrowserPane({
  ...
  const urlInputStyle = useMemo(() => [styles.urlInput, URL_INPUT_OUTLINE_STYLE], []);
    ...
    <View style={styles.container}>
      <View style={styles.chromeRow}>
        ...
        <View style={styles.chromeRight}>
          ...
          >
            {selectorMode === "annotate" ? (
              <ThemedMousePointerActive size={16} />
            ) : (
              <ThemedMousePointerMuted size={16} />
            )}
          </ToolbarButton>
          <ToolbarButton
            label={
          ...
          >
            {selectorMode === "screenshot" ? (
              <ThemedCameraActive size={16} />
            ) : (
              <ThemedCameraMuted size={16} />
            )}
          </ToolbarButton>
        </View>
      </View>
      {browser?.lastError ? (
        <View style={styles.errorRow}>
          <Text numberOfLines={1} style={styles.metaError}>
            {browser.lastError}
          </Text>
        </View>
      ) : null}
