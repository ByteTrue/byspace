...
export function LocalDaemonSection() {
  ...
  const advancedSettingsIcon = useMemo(() => <ThemedArrowUpRight />, []);
  const copyIcon = useMemo(() => <ThemedCopy />, []);
  const fileTextIcon = useMemo(() => <ThemedFileText />, []);
  const activityIcon = useMemo(() => <ThemedActivity />, []);
...
const ADVANCED_DAEMON_SETTINGS_URL = "https://app.byspace.cc.cd/docs/configuration";
