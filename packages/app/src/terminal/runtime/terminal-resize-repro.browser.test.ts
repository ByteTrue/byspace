...
// Regression: "streaming pino log, resized the BySpace terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__byspaceTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (BySpace terminal)", () => {
