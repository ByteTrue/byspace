...
import type { ForgeSearchItem, ForgeSearchResponse } from "@bytetrue/protocol/messages";
...
const gitlabMr73: ForgeSearchItem = {
  ...
  number: 73,
  title: "Attach MR",
  url: "https://gitlab.com/acme/byspace/-/merge_requests/73",
  state: "opened",
  body: null,
  labels: [],
  projectPath: "acme/byspace",
  baseRefName: "main",
  headRefName: "feature",
};
...
const giteaIssue27: ForgeSearchItem = {
  ...
  url: "https://gitea.example.com/acme/byspace/issues/27",
  state: "open",
  body: null,
  labels: [],
};
...
describe("useComposerForgeAutoAttach", () => {
    ...
    const { result } = renderHook(
      () =>
        useHarness(client, {
          ...
          remote: "git@gitlab.com:acme/byspace.git",
        }),
      { wrapper: createWrapper() },
    );

    act(() => {
      result.current.setText("Review https://gitlab.com/acme/byspace/-/merge_requests/73/diffs");
    });
    await flushDebounce();
  ...
  it("adds a matching pasted self-hosted Gitea issue URL", async () => {
    ...
    const { result } = renderHook(
      () =>
        useHarness(client, {
          remote: "git@gitea.example.com:acme/byspace.git",
        }),
      { wrapper: createWrapper() },
    );

    act(() => {
      result.current.setText("See https://gitea.example.com/acme/byspace/issues/27");
    });
    await flushDebounce();
  ...
  it("preserves pull request order across lookup batches", async () => {
    ...
    act(() => {
      result.current.setText(
        "Review https://github.com/acme/byspace/pull/101 and https://github.com/acme/byspace/pull/202",
      );
    });
    expect(result.current.isResolving).toBe(true);
    await flushDebounce();
  ...
  it("uses the latest pull request order when URLs are reordered during debounce", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/byspace/pull/202 and https://github.com/acme/byspace/pull/101",
      );
    });
    await flushDebounce();
  ...
  it("does not report a buffered pull request after its URL is removed", async () => {
    ...
    act(() => {
      result.current.setText("Still https://github.com/acme/byspace/pull/101");
    });
    await flushDebounce();
  ...
  it("stays resolving when an unrelated attachment is added during lookup", async () => {
    ...
    act(() => {
      ...
      result.current.setText("Review https://github.com/acme/byspace/pull/101 please");
    });
