...
import { CalendarClock, History, Network, Plus, Search } from "lucide-react-native";
...
function SidebarCollabRow({ onBeforeNavigate }: SidebarNavRowProps) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const handlePress = useCallback(() => {
    onBeforeNavigate?.();
    router.push(buildCollabRoute());
  }, [onBeforeNavigate]);

  return (
    <SidebarHeaderRow
      icon={Network}
      label={t(builtinSidebarNavLabelKey("collab"))}
      onPress={handlePress}
      isActive={pathname.includes("/collab")}
      testID="sidebar-collab"
      variant="compact"
    />
  );
}
