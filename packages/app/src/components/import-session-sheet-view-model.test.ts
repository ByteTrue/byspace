...
import type { FetchRecentProviderSessionEntry } from "@byspace/client/internal/daemon-client";
import {
...
describe("resolveDirectoryLabel", () => {
  const projects = [
    { rootPath: "/home/me/byspace", name: "byspace" },
    { rootPath: "/home/me/byspace/packages/app", name: "byspace app" },
  ];

  it("names the project root after the project alone", () => {
    expect(resolveDirectoryLabel("/home/me/byspace", projects)).toEqual({ name: "byspace" });
  });

  it("qualifies a worktree under the project with its path below the root", () => {
    expect(
      resolveDirectoryLabel("/home/me/byspace/.dev/worktrees/abc/zebra", [projects[0]!]),
    ).toEqual({ name: "byspace", detail: ".dev/worktrees/abc/zebra" });
  });

  it("tells two worktrees of the same project apart", () => {
    const zebra = resolveDirectoryLabel("/home/me/byspace/.dev/worktrees/abc/zebra", [
      projects[0]!,
    ]);
    const otter = resolveDirectoryLabel("/home/me/byspace/.dev/worktrees/def/otter", [
      projects[0]!,
    ]);
  ...
  it("picks the most specific project root containing the directory", () => {
    expect(resolveDirectoryLabel("/home/me/byspace/packages/app/src", projects)).toEqual({
      name: "byspace app",
      detail: "src",
    });
  });
  ...
  it("ignores trailing slashes on both sides", () => {
    expect(
      resolveDirectoryLabel("/home/me/byspace/", [{ rootPath: "/home/me/byspace/", name: "p" }]),
    ).toEqual({ name: "p" });
  });

  it("does not match a project root that is only a string prefix", () => {
    expect(resolveDirectoryLabel("/home/me/byspace-fork", projects)).toEqual({
      name: "/home/me/byspace-fork",
    });
  });
...
describe("formatDirectoryLabel", () => {
  it("shows the project name alone at its root", () => {
    expect(formatDirectoryLabel({ name: "byspace" })).toBe("byspace");
  });

  it("appends the path under the root so worktrees of one project read apart", () => {
    expect(formatDirectoryLabel({ name: "byspace", detail: ".dev/worktrees/zebra" })).toBe(
      "byspace · .dev/worktrees/zebra",
    );
  });
});
...
describe("resolveImportTarget", () => {
  it("trusts a scoped listing, whose rows the daemon already matched realpath-aware", () => {
    expect(
      resolveImportTarget({
        entryCwd: "/private/repo/byspace",
        workspaceCwd: "/repo/byspace",
        workspaceId: "ws-1",
  ...
  it("treats a sheet with no workspace as cross-workspace", () => {
    expect(resolveImportTarget({ entryCwd: "/repo/byspace", isScopedListing: false })).toEqual({
      crossWorkspace: true,
    });
  });
});
