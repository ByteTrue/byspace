...
} from "@byspace/protocol/provider-snapshot-codec";
import { CompactProviderSnapshotSchema } from "@byspace/protocol/messages";
import { z } from "zod";
import type { GetProvidersSnapshotResponseMessage } from "@byspace/protocol/messages";
type SnapshotPayload = GetProvidersSnapshotResponseMessage["payload"];

const CACHE_VERSION = 2;
const CACHE_KEY_PREFIX = "@byspace/provider-snapshot/v2";
const CACHE_INDEX_KEY = "@byspace/provider-snapshot-index/v2";
...
export function createProviderSnapshotCache(
  ...
  async function reconcileCache(): Promise<void> {
    ...
    const legacyKeys = allKeys.filter(
      (key) =>
        key.startsWith("@byspace/provider-snapshot/v1:") ||
        key === "@byspace/provider-snapshot-index/v1" ||
        key === CACHE_INDEX_KEY,
    );
