...
export function PullRequestPane({
  ...
  // COMPAT(githubCheckDetailsRpc): added in BySpace v0.1.2, remove after 2027-01-18 once
  ...
  const checkDetailsRoute = resolveForgeCapabilities({
    forge: data.forge,
    features: {
      forgeCheckDetails: canFetchForgeCheckDetails,
      githubCheckDetails: canFetchGitHubCheckDetails,
    },
  }).checkDetails;
    ...
    async (check: PrPaneCheck) => {
      ...
      try {
        ...
        const canFetchDetail =
          checkDetailsRoute === "forge" ||
          (checkDetailsRoute === "legacy-github" && ref?.checkRunId !== undefined);
        if (
          canFetchDetail &&
          daemonClient &&
          ...
          try {
            let payload;
            if (checkDetailsRoute === "forge") {
              payload = await daemonClient.checkoutForgeGetCheckDetails({
                cwd,
            ...
            } else if (ref.checkRunId !== undefined) {
