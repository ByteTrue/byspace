...
describe("parseForgeRef", () => {
  it.each([
    [
      ...
      "https://github.com/bytetrue/byspace/pull/994/files?diff=split#discussion_r123",
      { kind: "change_request", number: 994 },
    ],
    [
      "GitHub issue",
      "https://github.com/bytetrue/byspace.git",
      "https://github.com/bytetrue/byspace/issues/456",
  ...
  it("canonicalizes a cloud SSH alias to its web host", () => {
    expect(
      parseForgeRef(
        ...
        "ssh://git@ssh.github.com/bytetrue/byspace.git",
      ),
  ...
  it("does not apply another forge's route grammar to a known cloud host", () => {
    expect(
      parseForgeRef(
        "https://github.com/bytetrue/byspace/pulls/31",
        "git@github.com:bytetrue/byspace.git",
      ),
    ).toBeNull();
    expect(
...
describe("extractForgeRefs", () => {
  ...
  it("returns no references without text or a valid remote", () => {
    expect(extractForgeRefs("", "git@github.com:bytetrue/byspace.git")).toEqual([]);
    expect(extractForgeRefs("https://github.com/bytetrue/byspace/pull/1", null)).toEqual([]);
  });
});
