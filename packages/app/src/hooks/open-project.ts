...
export interface CloneGithubProjectDirectlyInput extends ProjectRegistrationCallbacks {
  ...
  client:
    | (Pick<DaemonClient, "cloneGithubProject"> &
        Partial<Pick<DaemonClient, "cloneGithubWorkspace">>)
    | null;
  legacyClone?: boolean;
}
...
export async function cloneGithubProjectDirectly(
  ...
  if (input.legacyClone) {
    if (!input.client.cloneGithubWorkspace) {
      return { ok: false, errorCode: null, error: "Host requires a compatible clone client" };
    }
    ...
    const project: WorkspaceProjectDescriptorPayload = {
      projectId: payload.workspace.projectId,
      projectDisplayName: payload.workspace.projectDisplayName,
      projectCustomName: payload.workspace.projectCustomName,
      projectRootPath: payload.workspace.projectRootPath,
      projectKind: payload.workspace.projectKind,
    };
