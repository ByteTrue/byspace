...
RELAY_EXPORTS="$REPO_ROOT/node_modules/@bytetrue/byspace-relay/dist/e2ee.js"
FIXTURE_ROOT=""
...
export BYSPACE_MAESTRO_DAEMON_HEALTH_URL="${BYSPACE_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6777/api/health}"
...
if ! curl --fail --silent --show-error --max-time 3 "$BYSPACE_MAESTRO_DAEMON_HEALTH_URL" >/dev/null; then
  echo "BySpace daemon is unavailable at $BYSPACE_MAESTRO_DAEMON_HEALTH_URL" >&2
  exit 1
fi

FIXTURE_ROOT="$(mktemp -d /tmp/byspace-sidebar-drag-fixture-XXXXXX)"
export BYSPACE_MAESTRO_DRAG_A_NAME="000-byspace-drag-a-$(basename "$FIXTURE_ROOT")"
export BYSPACE_MAESTRO_DRAG_B_NAME="001-byspace-drag-b-$(basename "$FIXTURE_ROOT")"
export BYSPACE_MAESTRO_DRAG_Z_NAME="zzz-byspace-drag-z-$(basename "$FIXTURE_ROOT")"

mkdir -p "$OUT_DIR/flows"

if [ -z "${BYSPACE_MAESTRO_IOS_UDID:-}" ]; then
  export BYSPACE_MAESTRO_IOS_UDID
  BYSPACE_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: sh\.byspace$/appId: $ENV{BYSPACE_MAESTRO_APP_ID}/m;
    ...
    s/\$\{BYSPACE_MAESTRO_DRAG_A_NAME\}/$ENV{BYSPACE_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{BYSPACE_MAESTRO_DRAG_B_NAME\}/$ENV{BYSPACE_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{BYSPACE_MAESTRO_DRAG_Z_NAME\}/$ENV{BYSPACE_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$BYSPACE_MAESTRO_DRAG_A_NAME" \
  "$BYSPACE_MAESTRO_DRAG_B_NAME" \
  "$BYSPACE_MAESTRO_DRAG_Z_NAME"; do
  ...
  git -C "$project_path" config user.email "maestro@bytetrue.local"
...
const client = new DaemonClient({
  url: process.env.BYSPACE_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.BYSPACE_MAESTRO_DRAG_A_NAME,
  process.env.BYSPACE_MAESTRO_DRAG_B_NAME,
  process.env.BYSPACE_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.BYSPACE_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $BYSPACE_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$BYSPACE_MAESTRO_IOS_UDID")
