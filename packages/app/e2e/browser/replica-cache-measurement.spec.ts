...
const RUN_MEASUREMENT = process.env.BYSPACE_REPLICA_CACHE_MEASUREMENT === "1";
const TARGET_URL = process.env.BYSPACE_REPLICA_CACHE_MEASUREMENT_URL;
const REPORT_PATH = process.env.BYSPACE_REPLICA_CACHE_MEASUREMENT_REPORT;
const OBSERVATION_MS = Number(process.env.BYSPACE_REPLICA_CACHE_MEASUREMENT_MS ?? 60_000);
const HYDRATED_SELECTOR =
  process.env.BYSPACE_REPLICA_CACHE_HYDRATED_SELECTOR ??
...
measurementDescribe("Replica cache live measurement", () => {
  test("observes persistence and cached restore without mutating the daemon", async ({
    ...
    expect(TARGET_URL, "BYSPACE_REPLICA_CACHE_MEASUREMENT_URL must be a full app URL").toBeTruthy();
    expect(REPORT_PATH, "BYSPACE_REPLICA_CACHE_MEASUREMENT_REPORT must be set").toBeTruthy();
