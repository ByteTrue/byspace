...
import { ensureSidePanel, waitForWorkspaceTabsVisible } from "../helpers/workspace-tabs";
  ...
  test("reveals empty, launches a view, and hides back out of the way", async ({
    ...
    try {
      ...
      await test.step("closing its final tab hides the Side panel, and it reopens empty", async () => {
        ...
        await page.getByTestId("workspace-working-diff-close").filter({ visible: true }).click();
