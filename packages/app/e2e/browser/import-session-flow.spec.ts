...
const SCREENSHOT_DIRECTORY = path.join(
  ...
  ".byspace/plans/import-session-ux",
);
const claudeConfigDirectory = mkdtempSync(path.join(tmpdir(), "byspace-import-flow-claude-"));
...
test.beforeAll(async () => {
  ...
  const repo = await createTempGitRepo("isf-", {
    originUrl: "https://github.com/byspace-e2e/import-session-fixture.git",
  });
