 ...
 * instead of racing real OS focus. Visibility listeners are module-lifetime, so the first terminal
 * is retained only because it models the reported user sequence; it is not required to keep
 * focus observation mounted.
 *
...
async function readRenderedTerminalSize(page: Page): Promise<RenderedTerminalSize | null> {
  return await page.evaluate(() => {
    const terminal = (window as Window & { __byspaceTerminal?: { rows: number; cols: number } })
      .__byspaceTerminal;
...
test.describe("terminal PTY size claim under lost window focus", () => {
  ...
  test("a terminal created while the window is blurred claims its size when focus returns", async ({
    ...
    // A terminal is already open because this models the reported user sequence. Wait for the
    // daemon to know about it rather than sleeping: if it were missing from this snapshot it would
    // be misread as "new" below.
    ...
    // __byspaceTerminal points at the most recently mounted xterm — the new terminal.
