...
export function observeForkAttachment(page: Page): {
  ...
  waitForRequestBoundary: () => Promise<{ cursor: boolean; messageId: boolean }>;
} {
  const attachmentTexts: string[] = [];
  const requestBoundaries: { cursor: boolean; messageId: boolean }[] = [];
  ...
  page.on("websocket", (socket) => {
    ...
    socket.on("framesent", ({ payload }) => {
      ...
      if (message?.type === "agent.fork_context.request") {
        requestBoundaries.push({
          cursor: Object.hasOwn(message, "boundaryCursor"),
          messageId: Object.hasOwn(message, "boundaryMessageId"),
        });
      }
    });
  ...
  return {
    ...
    async waitForRequestBoundary() {
      await expect.poll(() => requestBoundaries.length, { timeout: 30_000 }).toBeGreaterThan(0);
      const boundary = requestBoundaries.at(-1);
      if (!boundary) throw new Error("Expected a fork context request");
      return boundary;
    },
  };
}
