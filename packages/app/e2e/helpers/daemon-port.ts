 ...
 *
 * The port-6777 guard is a hard guardrail: 6777 is the developer's default
 * daemon, which manages real agents. The e2e port is never legitimately 6777,
...
export function getE2EDaemonPort(): string {
  ...
  if (port === "6777") {
    throw new Error("E2E_DAEMON_PORT must not point at the developer daemon (6777).");
  }
  return port;
}
