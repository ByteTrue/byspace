...
test.describe("New workspace flow", () => {
  ...
  test("opens Add project from the project picker", async ({ page }) => {
  ...
  test("pasted GitHub PR replaces a selected branch and creates its worktree", async ({ page }) => {
    ...
    await pasteGithubPrUrl(page, pr.url);
    ...
    expect(worktree.projectKey).toBe(openedProject.projectKey);
