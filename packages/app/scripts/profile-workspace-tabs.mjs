...
const serverId = requiredEnv("BYSPACE_PROFILE_SERVER_ID");
const workspaceId = requiredEnv("BYSPACE_PROFILE_WORKSPACE_ID");
const agentId = requiredEnv("BYSPACE_PROFILE_AGENT_ID");
const switchCount = Number(process.env.BYSPACE_PROFILE_SWITCH_COUNT ?? 6);
const switchWaitMs = Number(process.env.BYSPACE_PROFILE_SWITCH_WAIT_MS ?? 250);
const idleWaitMs = Number(process.env.BYSPACE_PROFILE_IDLE_WAIT_MS ?? 0);
...
const createdTerminalId = process.env.BYSPACE_PROFILE_TERMINAL_ID ? null : createTerminal();
const terminalId = process.env.BYSPACE_PROFILE_TERMINAL_ID ?? createdTerminalId;
...
async function main() {
  ...
  try {
    ...
    const samples = await page.evaluate(() => globalThis.__BYSPACE_RENDER_PROFILE__ ?? []);
    const reasons = await page.evaluate(() => globalThis.__BYSPACE_RENDER_PROFILE_REASONS__ ?? {});
    console.log(
      JSON.stringify(
        {
          appUrl: baseUrl,
