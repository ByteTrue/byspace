...
import { writeBySpaceWorktreeMetadata } from "./worktree-metadata.js";
...
describe("listCheckoutCommits", () => {
  it("lists current and retained base commits with remote flags and file stats", async () => {
    ...
    expect(
      commits.map(({ subject, isOnRemote, isOnBase }) => ({ subject, isOnRemote, isOnBase })),
    ).toEqual([
      { subject: "Add bar", isOnRemote: false, isOnBase: false },
      { subject: "Add foo", isOnRemote: true, isOnBase: false },
      { subject: "initial", isOnRemote: true, isOnBase: true },
    ]);
  ...
  it("keeps recent history when the saved base branch no longer exists", async () => {
    ...
    writeBySpaceWorktreeMetadata(worktreeDir, { baseRefName: "deleted-base" });
  ...
  it("shows every workspace commit followed by at most 10 base commits", async () => {
    ...
    expect(commits.slice(0, 24).every((entry) => !entry.isOnBase)).toBe(true);
    expect(commits.slice(24).map((entry) => entry.subject)).toEqual(
      Array.from({ length: 10 }, (_, index) => `Base ${14 - index}`),
    );
    expect(commits.slice(24).every((entry) => entry.isOnBase)).toBe(true);
  });
  ...
  it("classifies renamed, modified, and deleted files", async () => {
    const { repoDir } = initRepoOnMain();
    git(["checkout", "-b", "feature"], repoDir);
    commitFile(repoDir, "original.txt", "content\n", "Add original");
    git(["mv", "original.txt", "renamed.txt"], repoDir);
    commit(repoDir, "Rename file");
    expect((await listCheckoutCommits({ cwd: repoDir })).commits[0]?.files).toEqual([
