...
describe("path equivalence", () => {
  ...
  test("checks POSIX root containment without prefix false positives", () => {
    expect(isPathInsideRoot("/opt/byspace", "/opt/byspace/node_modules/@bytetrue/server")).toBe(
      true,
    );
  ...
  test("checks Windows root containment case-insensitively", () => {
    expect(
      isPathInsideRoot("C:\\BySpace\\node_modules", "c:/byspace/node_modules/@bytetrue/server"),
    ).toBe(true);
