...
import type { TerminalCell, TerminalState } from "@byspace/protocol/messages";
...
function createResumableTerminal(input: { backlogChars?: number; replayPreamble?: string }): {
  ...
  const terminal = {
    ...
    getReplayPreamble: () => input.replayPreamble ?? "",
    getTitle: () => undefined,
    getActivity: () => null,
    setActivity: vi.fn(),
    setTitle: vi.fn(),
...
function createResumeController(input: {
  ...
  return new TerminalSessionController({
    ...
    isPathWithinRoot: () => false,
    sessionLogger: createLogger(),
    getClientBufferedAmount: () => input.bufferedAmount ?? 0,
  });
}
...
describe("terminal-session-controller resume", () => {
  ...
  test("does not replay a cached input-mode preamble after a revision resume", async () => {
    const binaryFrames: TerminalStreamFrame[] = [];
    const { terminal, emit } = createResumableTerminal({
      replayPreamble: "\x1b[?2004h",
    });
    const controller = createResumeController({ terminal, binaryFrames });
    ...
    expect(binaryFrames.map((frame) => frame.opcode)).toEqual([TerminalStreamOpcode.Output]);
    expect(frameText(binaryFrames[0])).toBe("\x1b[?2004l");
  });

  test("falls back to a snapshot instead of resuming after output overflow", async () => {
    const binaryFrames: TerminalStreamFrame[] = [];
    const { terminal, emit } = createResumableTerminal({ backlogChars: 1_000_000 });
    ...
    await subscribeWithResume(controller, "req-1");
    binaryFrames.length = 0;
    emit("x".repeat(300_000));
    await new Promise((resolve) => setTimeout(resolve, 20));

    expect(binaryFrames.map((frame) => frame.opcode)).toEqual([TerminalStreamOpcode.Restore]);
  });

  test("resumes from the revision carried by the first snapshot", async () => {
    const binaryFrames: TerminalStreamFrame[] = [];
    const { terminal, emit } = createResumableTerminal({});
    const controller = createResumeController({ terminal, binaryFrames });
    ...
    await subscribeWithResume(controller, "req-1");
    await controller.dispatch({ type: "unsubscribe_terminal_request", terminalId: "term-1" });
    emit("missed-after-snapshot\n");

    binaryFrames.length = 0;
    await subscribeWithResume(controller, "req-2");

    expect(binaryFrames.map((frame) => frame.opcode)).toEqual([TerminalStreamOpcode.Output]);
    expect(frameText(binaryFrames[0])).toBe("missed-after-snapshot\n");
  });
  ...
  test("does not resume a terminal this client has never received", async () => {
    const binaryFrames: TerminalStreamFrame[] = [];
    const { terminal, emit } = createResumableTerminal({});
    emit("produced-before-any-subscription\n");
    const controller = createResumeController({ terminal, binaryFrames });
  ...
  test("clears the resume anchor when every client transport is unavailable", async () => {
    const binaryFrames: TerminalStreamFrame[] = [];
    const { terminal, emit } = createResumableTerminal({});
    const controller = createResumeController({ terminal, binaryFrames });

    await subscribeWithResume(controller, "req-1");
    controller.handleTransportUnavailable();
    emit("output-while-disconnected\n");
...
describe("terminal-session-controller subdirectory aggregation", () => {
  ...
  test("keeps nested workspace terminals out of the parent workspace terminal list", async () => {
    const rootCwd = "/work/repo";
    const worktreeCwd = "/work/repo/.dev/byspace-home/worktrees/hash/feature-a";
...
describe("terminal-session-controller backpressure snapshot fallback", () => {
  ...
  test("uses a high bufferedAmount socket to enter the snapshot backpressure path", async () => {
    const socket = {
      send() {
        return true;
      },
