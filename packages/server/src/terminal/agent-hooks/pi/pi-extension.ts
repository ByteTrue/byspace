...
function report(state) {
  const url =
    process.env.BYSPACE_TERMINAL_ACTIVITY_URL ?? process.env.BYSPACE_TERMINAL_ACTIVITY_URL;
  const terminalId = process.env.BYSPACE_TERMINAL_ID ?? process.env.BYSPACE_TERMINAL_ID;
  const token = process.env.BYSPACE_ACTIVITY_TOKEN ?? process.env.BYSPACE_ACTIVITY_TOKEN;
