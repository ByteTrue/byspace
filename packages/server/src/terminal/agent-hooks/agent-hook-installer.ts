...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${BYSPACE_HOOK_CLI:-\${BYSPACE_HOOK_CLI:-byspace}}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined BYSPACE_TERMINAL_ID (if defined BYSPACE_HOOK_CLI ("%BYSPACE_HOOK_CLI%" ${hookArgs}) else (if defined BYSPACE_HOOK_CLI ("%BYSPACE_HOOK_CLI%" ${hookArgs}) else (byspace ${hookArgs}))) else (exit /b 0)`;
}
