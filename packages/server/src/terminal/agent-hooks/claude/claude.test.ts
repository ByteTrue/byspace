...
describe("Claude terminal agent hooks", () => {
  it("installs registered provider hooks idempotently", () => {
    ...
    for (const event of provider.events) {
      ...
      expect(byspaceCommands[0]).toBe(
        `if [ -n "$BYSPACE_TERMINAL_ID" ]; then "\${BYSPACE_HOOK_CLI:-\${BYSPACE_HOOK_CLI:-byspace}}" hooks ${provider.id} ${event.event}; fi`,
      );
    }
  ...
  it("builds a minimal gated hook command", () => {
    ...
    expect(command).toBe(
      'if [ -n "$BYSPACE_TERMINAL_ID" ]; then "${BYSPACE_HOOK_CLI:-${BYSPACE_HOOK_CLI:-byspace}}" hooks claude UserPromptSubmit; fi',
    );
  });
