...
describe("Claude terminal agent hooks", () => {
  it("installs registered provider hooks idempotently", () => {
    ...
    for (const event of provider.events) {
      ...
      expect(paseoCommands[0]).toBe(
        `if [ -n "$PASEO_TERMINAL_ID" ]; then "\${BYSPACE_HOOK_CLI:-\${PASEO_HOOK_CLI:-byspace}}" hooks ${provider.id} ${event.event}; fi`,
      );
    }
  ...
  it("builds a minimal gated hook command", () => {
    ...
    expect(command).toBe(
      'if [ -n "$PASEO_TERMINAL_ID" ]; then "${BYSPACE_HOOK_CLI:-${PASEO_HOOK_CLI:-byspace}}" hooks claude UserPromptSubmit; fi',
    );
  });
