...
describe("OpenCode terminal agent hooks", () => {
  ...
  it.each(["succeeded", "failed", "interrupted"])(
    ...
    async (outcome) => {
      ...
      expect(commands).toEqual([
        ["byspace", "hooks", "opencode", "session.status.busy"],
        ["byspace", "hooks", "opencode", "session.status.idle"],
      ]);
    },
  );
  ...
  it("reports OpenCode 1 status and permission events through server()", async () => {
    ...
    expect(plugin.id).toBe("byspace-terminal-activity");
    expect(commands).toEqual([
      ["byspace", "hooks", "opencode", "session.status.busy"],
      ["byspace", "hooks", "opencode", "permission.asked"],
      ["byspace", "hooks", "opencode", "permission.replied"],
      ["byspace", "hooks", "opencode", "session.status.retry"],
      ["byspace", "hooks", "opencode", "session.status.idle"],
    ]);
  });
  ...
  it("preserves activity order when the host dispatches events concurrently", async () => {
    ...
    expect(commands).toEqual([["byspace", "hooks", "opencode", "session.status.busy"]]);
  ...
  it("keeps both generations inert outside BySpace terminals", async () => {
    const { plugin, commands } = loadInstalledPlugin("");
...
function loadInstalledPlugin(terminalId = "terminal-1", exited = Promise.resolve(0)) {
  const configDir = createTempDir("byspace-opencode-runtime-");
  ...
  const plugin: InstalledPlugin = runInNewContext(
    ...
    {
      AbortController,
      process: { env: { BYSPACE_TERMINAL_ID: terminalId } },
      Bun: {
