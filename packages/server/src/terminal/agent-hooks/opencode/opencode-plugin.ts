...
export const OPENCODE_PLUGIN_SOURCE = [
  ...
  "};",
  "",
  "function byspaceEventForV1(event) {",
  ...
  "",
  "export default {",
  '  id: "byspace-terminal-activity",',
  "  server() {",
  "    return {",
  "      event: async ({ event }) => {",
  "        const byspaceEvent = byspaceEventForV1(event);",
  "        if (byspaceEvent) await runBySpaceHook(byspaceEvent);",
  "      },",
  "    };",
  "  },",
  "  setup(ctx) {",
  ...
  "        const byspaceEvent = V2_EVENTS[event.type];",
  "        if (byspaceEvent) await runBySpaceHook(byspaceEvent);",
  "      }",
  "    })().catch(() => {});",
