...
import { parseGitRemoteLocation, type GitRemoteLocation } from "@bytetrue/protocol/git-remote";
...
export function createForgeResolver(options: CreateForgeResolverOptions = {}): ForgeResolver {
  ...
  // from outside BySpace — `git remote add`/`set-url` run in a terminal — is
  // still picked up within a cycle instead of staying pinned forever; a BySpace
