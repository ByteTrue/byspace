...
} from "@bytetrue/protocol/git-remote";
...
fragment BySpacePollPullRequest on PullRequest {
  number
  url
  title
  state
...
function buildBatchPullRequestStatusQuery(
  ...
  const aliases = entries.map(
    ...
    pullRequests(headRefName: ${graphqlString(
      ...
      nodes {
        ...BySpacePollPullRequest
      }
    }
  }`,
  );
  return `query BySpaceBatchPullRequestStatus {
  rateLimit {
    limit
    remaining
    resetAt
...
function buildBatchPullRequestChecksQuery(
  ...
  return `query BySpaceBatchPullRequestChecks {
  rateLimit {
    limit
    remaining
    resetAt
