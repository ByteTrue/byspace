...
describe("file uploads", () => {
  ...
  it("stores chunked upload bytes and returns an uploaded-file attachment", async () => {
    const byspaceHome = makeBySpaceHome();
    const uploads = new FileUploadStore({ byspaceHome });
    ...
    const path = join(byspaceHome, "uploads", "upload_req-upload", "notes.txt");
  ...
  it("rejects chunks beyond the declared size and removes the partial file", async () => {
    ...
    const uploadDir = join(byspaceHome, "uploads", "upload_req-overflow");
  ...
  it("preserves chunk order when frames arrive before earlier disk writes finish", async () => {
    ...
    expect(
      readFileSync(join(byspaceHome, "uploads", "upload_req-queued", "notes.txt"), "utf8"),
    ).toBe("hello world");
  });
  ...
  it("replaces duplicate upload starts without letting the old stale timeout evict the replacement", async () => {
    ...
    const byspaceHome = makeBySpaceHome();
    const uploads = new FileUploadStore({ byspaceHome, staleUploadTimeoutMs: 50 });
    ...
    const path = join(byspaceHome, "uploads", "upload_req-duplicate_2", "new.txt");
    await expect(uploads.receiveFrame(uploadBegins("req-duplicate"))).resolves.toBeNull();
  ...
  it("keeps an active upload alive beyond the initial stale timeout", async () => {
    ...
    const path = join(byspaceHome, "uploads", "upload_req-slow-active", "notes.txt");
...
function makeBySpaceHome(): string {
