...
/**
 * Trust-On-First-Use host key pinning for daemon-owned SSH tunnels.
 * Password auth hands the credential to whatever answers the connection, so
 * the server fingerprint is recorded on first use and verified on every later
 * connect; a mismatch is treated as an active attack and fails the tunnel.
 *
 * Key-path sessions (no password) keep using the system OpenSSH known_hosts
 * semantics through a per-connection managed known_hosts file.
 */

const KNOWN_HOSTS_FILENAME = "tunnel-known-hosts.json";
...
export function createFileKnownHostsStore(byspaceHomePath: string): KnownHostsStore {
  const filePath = join(byspaceHomePath, KNOWN_HOSTS_FILENAME);
  return {
    async load() {
      try {
    ...
    async save(entries) {
      mkdirSync(byspaceHomePath, { recursive: true });
