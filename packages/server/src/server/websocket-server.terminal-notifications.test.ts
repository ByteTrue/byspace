...
function connectClient(server: DaemonWebSocketServer, subscribed = true) {
...
describe("DaemonWebSocketServer terminal attention notifications", () => {
  ...
  it("falls back to the terminal name when capture fails", async () => {
    const { manager, emit } = createTerminalManager([], async () => {
  ...
  it("uses captured output for needs-input notifications", async () => {
    const { manager, emit } = createTerminalManager(["Permission requested"]);
    const { server } = createServer(manager);
    const ws = connectClient(server);
    ...
    expect(payload.body).toBe("Permission requested");
  });
