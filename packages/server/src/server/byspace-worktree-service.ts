...
import type { FirstAgentContext } from "@byspace/protocol/messages";
...
export async function createBySpaceWorktree(
  ...
  return runWithGitCommandPriority("high", () => createBySpaceWorktreeWithPriority(input, deps));
}

async function createBySpaceWorktreeWithPriority(
  input: CreateBySpaceWorktreeInput,
  deps: CreateBySpaceWorktreeDeps,
): Promise<CreateBySpaceWorktreeResult> {
