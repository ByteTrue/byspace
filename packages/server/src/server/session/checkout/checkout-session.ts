...
type LegacyGithubSearchResultItem = Omit<
  ForgeSearchResultItem,
  "kind" | "forge" | "projectPath"
> & {
  kind: "issue" | "pr";
};

function toLegacyGithubSearchItems(items: ForgeSearchResultItem[]): LegacyGithubSearchResultItem[] {
  return items.map(({ forge: _forge, projectPath: _projectPath, ...item }) => {
...
export class CheckoutSession {
  ...
  async handleCheckoutForgeGetCheckDetailsRequest(
    ...
    const changeRequestNumber =
      msg.type === "checkout.forge.get_check_details.request" ? msg.changeRequestNumber : undefined;
    const responseType =
  ...
  async handleForgeSearchRequest(
    ...
    try {
      const resolvedCwd = expandTilde(cwd);
      // COMPAT(githubSearchRpc): added in BySpace v0.1.2, remove after 2027-01-18 —
