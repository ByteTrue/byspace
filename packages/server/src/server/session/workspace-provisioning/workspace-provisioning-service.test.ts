...
test("runInImportWorkspace preserves an active project when an untargeted import fails", async () => {
  const cwd = path.join(tmpDir, "failed-import-active");
  mkdirSync(cwd);
  const project = await provisioning.findOrCreateProjectForDirectory(cwd);
  const previousProject = await projectRegistry.get(project.projectId);
