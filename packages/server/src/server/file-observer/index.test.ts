...
// Windows runners execute this stress test 2-3x slower than Linux and have
// flaked on it twice (PR #34, v0.14.0); the same churn logic is covered on
// ubuntu, so Windows skips only this stress case (issue 027).
...
async function createRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "byspace-file-observer-"));
  roots.add(root);
  return root;
}
