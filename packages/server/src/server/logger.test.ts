...
async function runLoggerFixture(source: string): Promise<{ stdout: string; stderr: string }> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "byspace-logger-fixture-"));
...
describe("resolveLogConfig", () => {
  const byspaceHome = "/tmp/byspace-logger-tests";

  it("defaults to stdout JSON without file logging", () => {
    const result = resolveLogConfig(undefined, { byspaceHome });
  ...
  it("keeps legacy level and format as stdout configuration", () => {
    const result = resolveLogConfig({ level: "warn", format: "pretty" }, { byspaceHome });
  ...
  it("enables file output only when log.file is present", () => {
    ...
    expect(resolveLogConfig(config, { byspaceHome })).toEqual({
      level: "debug",
      console: {
        level: "warn",
        format: "json",
      ...
      file: {
        level: "debug",
        path: path.resolve(byspaceHome, "logs", "programmatic.log"),
      },
    });
  });
  ...
  it("defaults file output to info when log.file is present without a level", () => {
    ...
    expect(resolveLogConfig(config, { byspaceHome })).toEqual({
      ...
      file: {
        level: "info",
        path: path.resolve(byspaceHome, "daemon.log"),
      },
    });
  });
});
...
describe("loadConfig logger config", () => {
  it("applies log format env at the config boundary", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "byspace-logger-config-"));
    ...
    const config = loadConfig(byspaceHome, {
      env: { BYSPACE_LOG_FORMAT: "pretty" },
    });

    expect(config.log?.format).toBe("pretty");
    expect(resolveLogConfig(config, { byspaceHome }).console.format).toBe("pretty");
  });
});

describe("createRootLogger", () => {
  it("includes the daemon version in child logger entries", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-logger-version-"));

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(undefined, { byspaceHome: ${JSON.stringify(byspaceHome)} });
  ...
  it("writes JSON to stdout by default and does not initialize file logging", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-logger-default-"));
    const missingLogDir = path.join(byspaceHome, "logs");
    ...
    expect(existsSync(path.join(byspaceHome, "daemon.log"))).toBe(false);
    expect(existsSync(missingLogDir)).toBe(false);
  });

  it("writes to an explicit file target without creating rotation files", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-logger-file-"));
    const logPath = path.join(byspaceHome, "logs", "programmatic.log");

    await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { byspaceHome: ${JSON.stringify(byspaceHome)} },
      );
  ...
  it("can disable file output for supervised workers", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-logger-no-worker-file-"));
    const logPath = path.join(byspaceHome, "daemon.log");

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { byspaceHome: ${JSON.stringify(byspaceHome)}, file: false },
      );
  ...
  it("keeps pretty output available as a format choice", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-logger-pretty-"));
