...
import { createWorktree, getBySpaceWorktreesRoot } from "../utils/worktree.js";
...
test("does not overwrite a committed exact-project config with source checkout edits", async () => {
  ...
  writeFileSync(path.join(sourceDir, "byspace.json"), committedConfig);
  commitAll(repoDir, "add subproject config");
  writeFileSync(
    path.join(sourceDir, "byspace.json"),
  ...
  expect(readFileSync(path.join(result.workspace.cwd, "byspace.json"), "utf8")).toBe(
    committedConfig,
  );
  expect(
...
test("removes a new worktree when its ref does not contain the selected project directory", async () => {
  ...
  const worktreePath = path.join(
    await getBySpaceWorktreesRoot(repoDir, byspaceHome),
    "missing-subproject",
  );
...
test("maps a nested cwd from an existing BySpace worktree into the next worktree", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const byspaceHome = path.join(tempDir, ".byspace");
  ...
  commitAll(repoDir, "add subproject");
  const deps = createDeps();
  const source = await createBySpaceWorktree(
    {
      cwd: repoDir,
...
test("rejects source checkout planning before creating a worktree", async () => {
  ...
  expect(existsSync(path.join(byspaceHome, "worktrees"))).toBe(false);
