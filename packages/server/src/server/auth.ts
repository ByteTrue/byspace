...
/** Full orchestration capability; caller metadata selects semantics, not authorization scope. */
export function isAgentCliTokenValid(
  auth: DaemonAuthConfig | undefined,
  token: string | null,
): boolean {
  if (!auth?.agentCliToken || !token) return false;
  const expected = Buffer.from(auth.agentCliToken);
...
export function extractWsBearerProtocol(value: string | undefined): string | null {
  ...
  for (const protocol of value.split(",")) {
    ...
    if (segments[0] === "byspace" && segments[1] === "bearer" && segments.length >= 3) {
      return trimmed;
    }
  }
...
const BEARER_AUTH_BYPASS_PATHS = new Set([
  ...
  // browser downloads: those trigger the download via an anchor navigation,
