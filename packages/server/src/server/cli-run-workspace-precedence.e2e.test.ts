...
// The daemon-level workspace contract that `byspace run` depends on: each
...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "byspace-cli-run-other-cwd-"));
