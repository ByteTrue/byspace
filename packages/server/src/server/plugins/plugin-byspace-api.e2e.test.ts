...
test("daemon config reload enables and disables configured plugins without restarting", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "byspace-reload-plugin-"));
  const byspaceHomeRoot = await mkdtemp(path.join(tmpdir(), "byspace-reload-home-"));
  const byspaceHome = path.join(byspaceHomeRoot, ".byspace");
  roots.push(pluginDirectory, byspaceHomeRoot);
  await writeFile(
    path.join(pluginDirectory, "byspace-plugin.json"),
  ...
  const configPath = path.join(daemon.byspaceHome, "config.json");
