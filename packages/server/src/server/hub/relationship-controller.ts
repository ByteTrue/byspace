...
export interface HubRelationshipStatus {
  ...
  // COMPAT(semanticHubPermissions): keep the legacy scope projection for older clients until 2027-09-15.
  scopes?: string[];
...
export class HubRelationshipController implements HubRelationshipManagement {
  ...
  status(): HubRelationshipStatus {
    return {
      ...
      // COMPAT(semanticHubPermissions): older clients require the execution scope in status responses.
      scopes: this.record?.relationship.permissions.includes("hub.execute")
        ? ["hub.execution.*"]
        : [],
