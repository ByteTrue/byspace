...
test("blocked setup remains readable on mixed-capability sockets sharing a session", async () => {
  await daemon.close();
  const root = await mkdtemp(path.join(os.tmpdir(), "byspace-blocked-compat-"));
  const projects = path.join(root, ".byspace", "projects");
  ...
  daemon = await createTestBySpaceDaemon({ byspaceHomeRoot: root });
...
test("plugin items are gated in provider child streams, child fetches, and rewind replay", async () => {
  ...
  daemon = await createTestBySpaceDaemon({ isDev: true, agentClients: { mock: provider } });
...
async function createAttentionWorkspace(client: DaemonClient): Promise<string> {
  const result = await client.createWorkspace({
    source: { kind: "directory", path: daemon.byspaceHome },
  });
