...
function measureLiveDelivery(messages: SessionOutboundMessage[]) {
  const timeline = messages.filter(
    (message) =>
      message.type === "agent_stream" && message.payload.event.type !== "attention_required",
  );
  const attention = messages.filter(
    (message) =>
      message.type === "agent_attention_required" ||
      (message.type === "agent_stream" && message.payload.event.type === "attention_required"),
  );
  const bytes = (selected: SessionOutboundMessage[]) =>
    selected.reduce(
      (total, message) =>
        total + Buffer.byteLength(JSON.stringify({ type: "session", message }), "utf8"),
      0,
    );
  return {
    timelineFrames: timeline.length,
    timelineBytes: bytes(timeline),
    attentionFrames: attention.length,
    attentionBytes: bytes(attention),
    agentIds: Array.from(
      new Set(
        timeline.map((message) => (message.type === "agent_stream" ? message.payload.agentId : "")),
      ),
    )
      .filter(Boolean)
      .sort(),
...
test("real WebSocket sessions enforce selective delivery, retained resets, downgrade, and dedicated attention", async () => {
  ...
  expect(capable.hasTimeline(agentC.id)).toBe(false);
  const capableTraffic = measureLiveDelivery(capable.messages);
  const legacyTraffic = measureLiveDelivery(legacy.messages);
  expect({ capable: capableTraffic, legacy: legacyTraffic }).toMatchObject({
    capable: {
      timelineFrames: 2,
      agentIds: [agentA.id, agentB.id].sort(),
    },
    legacy: {
      timelineFrames: 3,
      agentIds: [agentA.id, agentB.id, agentC.id].sort(),
    },
  });
  expect(capableTraffic.timelineBytes).toBeLessThan(legacyTraffic.timelineBytes);
  console.log(
    `SELECTIVE_TIMELINE_SERVER_TRAFFIC ${JSON.stringify({ capable: capableTraffic, legacy: legacyTraffic })}`,
  );
  ...
  const capableAttentionTraffic = measureLiveDelivery(capable.messages);
  const legacyAttentionTraffic = measureLiveDelivery(legacy.messages);
  expect(capableAttentionTraffic.attentionFrames).toBe(1);
  expect(legacyAttentionTraffic.attentionFrames).toBe(1);
  console.log(
    `SELECTIVE_TIMELINE_ATTENTION_TRAFFIC ${JSON.stringify({ capable: capableAttentionTraffic, legacy: legacyAttentionTraffic })}`,
  );
