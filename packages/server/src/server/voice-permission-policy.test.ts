...
describe("isVoicePermissionAllowed", () => {
  test("allows direct speak tool names across provider conventions", () => {
    ...
    expect(isVoicePermissionAllowed(buildRequest({ name: "byspace_voice.speak" }))).toBe(true);
    expect(isVoicePermissionAllowed(buildRequest({ name: "mcp__byspace_voice__speak" }))).toBe(
      true,
    );
  });

  test("denies non-speak tool names", () => {
    expect(isVoicePermissionAllowed(buildRequest({ name: "mcp__byspace__create_agent" }))).toBe(
      false,
    );
    expect(isVoicePermissionAllowed(buildRequest({ name: "byspace_create_agent" }))).toBe(false);
  });

  test("denies non-tool permission kinds", () => {
    const result = isVoicePermissionAllowed(
      buildRequest({ kind: "mode", name: "mcp__byspace__create_agent" }),
    );
  ...
  test("denies wrapper tools even when metadata references speak", () => {
    const denied = isVoicePermissionAllowed(
      buildRequest({
        ...
        metadata: {
          questions: [
            { question: "Allow codextool to call byspace_voice.speak for user feedback?" },
          ],
        },
      }),
    );
