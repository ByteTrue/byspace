...
import type {
  ...
  StoredSchedule,
  UpdateScheduleInput,
} from "@byspace/protocol/schedule/types";
...
import { PARENT_AGENT_ID_LABEL } from "@byspace/protocol/agent-labels";
import { MutableDaemonConfigSchema, type AgentProfile } from "@byspace/protocol/messages";
...
async function connectInMemoryMcpClient(server: Awaited<ReturnType<typeof createAgentMcpServer>>) {
  const client = new Client({ name: "byspace-test-client", version: "0.0.0" });
  ...
  it("applies provider policy to registered tools", async () => {
    ...
    const server = await createAgentMcpServer({
      ...
      providerSnapshotManager:
        new BoundaryProviderSnapshotManagerFake() as unknown as ProviderSnapshotManager,
      callerAgentId: "agent-1",
      byspaceToolPolicy: { disabledTools: ["create_workspace"] },
      logger,
    });
  ...
  it("filters policy-disabled tools from MCP listing and calls", async () => {
    ...
    const server = await createAgentMcpServer({
      ...
      providerSnapshotManager:
        new BoundaryProviderSnapshotManagerFake() as unknown as ProviderSnapshotManager,
      callerAgentId: "agent-1",
      byspaceToolPolicy: { disabledTools: ["list_agents", "create_workspace"] },
      logger,
    });
    const client = await connectInMemoryMcpClient(server);
...
describe("create_agent MCP tool", () => {
  ...
  it("creates a worktree workspace from a project root without a path", async () => {
    ...
    const receivedInputs: CreateBySpaceWorktreeInput[] = [];
    const createBySpaceWorktree: CreateBySpaceWorktreeWorkflowFn = async (input) => {
  ...
  it("preserves branch checkout and pull request checkout workspace modes", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const createBySpaceWorktree = vi.fn(async (input: CreateBySpaceWorktreeInput) => ({
      worktree: {
    ...
    expect(createBySpaceWorktree).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        action: "checkout",
  ...
  it("lists active workspace descriptors", async () => {
    ...
    const workspace = createPersistedWorkspaceRecord({
      ...
      projectId: "project-1",
      cwd: "/tmp/byspace/worktrees/repo/feature",
      kind: "worktree",
      displayName: "feature",
