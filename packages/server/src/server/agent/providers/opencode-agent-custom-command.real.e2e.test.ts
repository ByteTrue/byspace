...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through BySpace using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "byspace-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: BySpace issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
