...
describe("Claude SDK env", () => {
  test("forwards launch-context env through Claude process env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        BYSPACE_AGENT_ID: "00000000-0000-4000-8000-000000000201",
        BYSPACE_TEST_FLAG: "launch-value",
      },
    };
    ...
    try {
      ...
      expect(capturedEnv?.BYSPACE_AGENT_ID).toBe(launchContext.env?.BYSPACE_AGENT_ID);
      expect(capturedEnv?.BYSPACE_TEST_FLAG).toBe(launchContext.env?.BYSPACE_TEST_FLAG);
    } finally {
      await session.close();
    }
  });
  ...
  test("forwards launch-context env through Claude resume env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        BYSPACE_AGENT_ID: "00000000-0000-4000-8000-000000000202",
        BYSPACE_TEST_FLAG: "resume-launch-value",
      },
    };
    const queryFactory = vi.fn(({ options }: ClaudeQueryInput) => {
      capturedEnv = options.env;
