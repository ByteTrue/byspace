...
describe("codex agent commands E2E", () => {
  ...
  test("lists available slash commands for a codex agent", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "Say hello to $NAME and then output exactly BYSPACE_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "When invoked, respond with exactly BYSPACE_SKILL_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
  ...
  test("executes a custom prompt via normal sendMessage path (prompts:*)", async () => {
    ...
    const promptsDir = path.join(codexHome, "prompts");
    mkdirSync(promptsDir, { recursive: true });
    const promptPath = path.join(promptsDir, "byspace-test-sayok.md");
    writeFileSync(
      promptPath,
      [
        "---",
        ...
        "---",
        "",
        "Output exactly: BYSPACE_OK $NAME",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    await ctx.client.sendMessage(agent.id, "/prompts:byspace-test-sayok NAME=world");
    const state = await ctx.client.waitForFinish(agent.id, 30_000);

    expect(state.status).toBe("idle");
    expect(state.lastMessage).toContain("BYSPACE_OK");
  ...
  test("sendMessage routes known slash commands through standard run/stream", async () => {
    ...
    mkdirSync(promptsDir, { recursive: true });
    writeFileSync(
      path.join(promptsDir, "byspace-test-sayok.md"),
      ["---", "description: Say OK", "---", "", "Output exactly: BYSPACE_OK", ""].join("\n"),
      "utf8",
    );
    process.env.CODEX_HOME = codexHome;
    ...
    try {
      ...
      expect(state.status).toBe("idle");
      expect(state.lastMessage).toContain("BYSPACE_OK byspace-test-sayok");
    } finally {
      if (prevCodexHome === undefined) {
        delete process.env.CODEX_HOME;
      } else {
  ...
  test("sendMessage keeps unknown slash input as plain prompt text", async () => {
    ...
    expect(state.lastMessage).not.toContain("BYSPACE_SKILL_OK");
  }, 30_000);
