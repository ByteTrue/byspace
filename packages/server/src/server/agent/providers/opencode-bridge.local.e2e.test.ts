...
test("real OpenCode server shares one process while shell.env stays session-scoped", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "byspace-opencode-real-"));
  const firstCwd = path.join(root, "first");
  const secondCwd = path.join(root, "second");
  const logger = createTestLogger();
  const bridge = new OpenCodeBridge({ byspaceHome: root, logger });
  await bridge.start();
  ...
  try {
    ...
    first = await client.createSession(
      { provider: "opencode", cwd: firstCwd, model: "opencode/big-pickle", modeId: "build" },
      {
        ...
        env: { BYSPACE_AGENT_ID: "real-agent-one", BYSPACE_AGENT_CWD: firstCwd },
        byspaceTools: firstTools,
      },
    ...
    second = await client.createSession(
      { provider: "opencode", cwd: secondCwd },
      {
        ...
        env: { BYSPACE_AGENT_ID: "real-agent-two", BYSPACE_AGENT_CWD: secondCwd },
        byspaceTools: secondTools,
      },
      { persistSession: false },
    );
    ...
    const [firstShell, secondShell] = await Promise.all([
      sdk.session.shell({
        ...
        directory: firstCwd,
        agent: "build",
        command: 'printf "%s|%s" "$BYSPACE_AGENT_ID" "$BYSPACE_AGENT_CWD"',
      }),
      sdk.session.shell({
    ...
    const agentResult = await first.run(
      [
        "Use the bash tool to run: env | grep -E '^(BYSPACE_AGENT_ID|BYSPACE_AGENT_CWD)='",
    ...
    const callerResult = await first.run(
      [
        "Use the byspace_report_caller_agent_id tool to read your BySpace caller agent ID.",
    ...
    expect(callerResult.timeline).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "tool_call",
          name: "byspace_report_caller_agent_id",
          status: "completed",
        }),
      ]),
    );
...
function createCallerCatalog(callerAgentId: string): BySpaceToolCatalog {
  const tool = {
    name: "report_caller_agent_id",
    title: "Report BySpace caller agent ID",
    description: "Returns the caller agent ID assigned by BySpace.",
    inputSchema: {},
    async handler() {
