...
function createCatalog(): BySpaceToolCatalog {
  const tool = {
    name: "echo_context",
    title: "Echo context",
...
describe("OpenCodeBridge", () => {
  test("loads packaged bundle bytes without invoking source compilation", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "byspace-opencode-artifact-"));
  ...
  test("serves authenticated session context and caller-scoped tools", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-opencode-bridge-"));
    temporaryDirectories.push(byspaceHome);
    const catalog = createCatalog();
    const bridge = new OpenCodeBridge({ byspaceHome, logger: createTestLogger() });
    await bridge.start();
    bridge.setManifestCatalog(catalog);
    const release = bridge.bindSession({
      sessionId: "ses_one",
      env: {
        BYSPACE_AGENT_ID: "agent-one",
        BYSPACE_AGENT_CWD: "/workspace/one",
        CUSTOM_VALUE: "one",
      },
      tools: catalog,
    });
    ...
    try {
      ...
      await expect(
        hooks.tool.byspace_echo_context.execute(
      ...
      expect(pluginError).toHaveBeenCalledWith(
        "[byspace-opencode-plugin] shell.env failed",
  ...
  test("preserves user OpenCode config while installing one content-addressed plugin", async () => {
    const byspaceHome = await mkdtemp(path.join(tmpdir(), "byspace-opencode-bridge-config-"));
    temporaryDirectories.push(byspaceHome);
    const bridge = new OpenCodeBridge({ byspaceHome, logger: createTestLogger() });
    await bridge.start();
    ...
    try {
      ...
      expect(config.plugin[1]?.[0]).toMatch(/byspace-[a-f0-9]{64}\.mjs$/);
    } finally {
      await bridge.close();
    }
  });
