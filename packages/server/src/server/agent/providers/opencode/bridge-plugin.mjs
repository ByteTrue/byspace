...
export default async function byspacePlugin(input, options) {
  const request = async (pathname, init) => {
    ...
    if (!response.ok) {
      const error = new Error(
        payload.error ?? `BySpace OpenCode bridge failed: ${response.status}`,
      );
  ...
  for (const definition of manifest.tools ?? []) {
    tools[`byspace_${definition.name}`] = {
      ...
      execute: async (args, context) => {
        ...
        return {
          ...
          metadata: { byspaceTool: definition.name },
        };
      },
    };
  }
...
function logPluginError(stage, context, error) {
  console.error(`[byspace-opencode-plugin] ${stage} failed`, {
    ...context,
