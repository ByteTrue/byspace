...
import { AgentProfileSchema } from "@byspace/protocol/messages";
...
import { everyMsToFiveFieldCron } from "@byspace/protocol/schedule/cadence";
...
import { isBySpaceToolEnabled } from "../byspace-tool-policy.js";
...
export function createBySpaceToolCatalog(options: BySpaceToolHostDependencies): BySpaceToolCatalog {
  ...
  const registerTool = (
    ...
    if (!isBySpaceToolEnabled(options.byspaceToolPolicy, name)) {
      return;
    }
    tools.set(name, {
      name,
