...
interface RegisteredTool {
  ...
  handler: (args: unknown, context: BySpaceToolExecutionContext) => Promise<BySpaceToolResult>;
}
...
class BrowserToolHarness {
  ...
  public async execute(name: string, input: unknown): Promise<BySpaceToolResult> {
...
function schemaFor(inputSchema: BySpaceToolConfig["inputSchema"]): z.ZodType {
  if (!inputSchema) {
...
const routedToolCases = [
  ...
  content: BySpaceToolResult["content"];
}>;
...
describe("registerBrowserTools", () => {
  ...
  test("list tabs sends workspace in the request envelope", async () => {
    ...
    expect(response.content).toEqual([
      {
        type: "text",
        text: `Found 1 BySpace browser tab. Use these browserId values for tab-scoped browser tools.\n- browserId=${BROWSER_ID} active title="Example" url=https://example.com`,
      },
    ]);
  });
