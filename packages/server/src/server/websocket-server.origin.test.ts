...
describe("isWebSocketSameOrigin", () => {
  test("accepts exact same-origin websocket upgrades", () => {
    expect(isWebSocketSameOrigin("http://localhost:6777", "localhost:6777")).toBe(true);
    expect(isWebSocketSameOrigin("https://byspace.example.com", "byspace.example.com")).toBe(true);
  });
  ...
  test("rejects loopback aliases on different ports", () => {
    expect(isWebSocketSameOrigin("http://127.0.0.1:32775", "localhost:6777")).toBe(false);
  });
  ...
  test("rejects non-loopback cross-origin upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("http://127.0.0.1:32775", "byspace.example.com:32775")).toBe(
      false,
    );
  });
});
