...
import { createClientChannel, type Transport } from "@byspace/relay/e2ee";
import {
  deriveSharedKey,
  decrypt,
  encrypt,
...
import { WSOutboundMessageSchema } from "@byspace/protocol/messages";
...
(shouldRunRelayE2e ? describe : describe.skip)("Relay transport (E2EE) - daemon E2E", () => {
  ...
  const startRelay = async (options: { useLocalRelay?: boolean } = {}) => {
    ...
    if (options.useLocalRelay) {
      relayArgs.push("--var", "BYSPACE_RELAY_UPSTREAM:");
    }
