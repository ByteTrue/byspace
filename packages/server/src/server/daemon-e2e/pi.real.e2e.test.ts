...
async function withConnectedPiDaemon(
  run: (context: { client: DaemonClient; daemon: TestBySpaceDaemon }) => Promise<void>,
): Promise<void> {
...
test(
  "real Pi daemon composes project and BySpace system prompts",
  async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          "When the user says BYSPACE_SYSTEM_PROMPT_PROBE, reply with exactly two tokens:",
          "PROJECT_PROMPT followed by the value of BYSPACE_PROMPT_TOKEN from later system instructions.",
          "If no BYSPACE_PROMPT_TOKEN exists, use MISSING as the second token.",
        ].join("\n"),
      );
      ...
      await withConnectedPiDaemon(async ({ client }) => {
        const agent = await client.createAgent({
          ...
          systemPrompt:
            "BYSPACE_PROMPT_TOKEN is BYSPACE_PROMPT. Follow the project instruction for BYSPACE_SYSTEM_PROMPT_PROBE.",
        });

        await client.sendMessage(agent.id, "BYSPACE_SYSTEM_PROMPT_PROBE");
        ...
        const response = items
          ...
          .join("")
          .trim();
        expect(response).toBe("PROJECT_PROMPT BYSPACE_PROMPT");
      });
    } finally {
...
test(
  ...
  async () => {
    ...
    const firstPrompt = "BYSPACE_PI_ENTRY_ID_FIRST. Reply exactly: first-ok";
    const secondPrompt = "BYSPACE_PI_ENTRY_ID_SECOND. Reply exactly: second-ok";
