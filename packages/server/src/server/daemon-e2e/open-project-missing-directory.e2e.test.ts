...
beforeEach(() => {
  previousSupervised = process.env.BYSPACE_SUPERVISED;
...
function restoreSupervisedEnv(): void {
  if (previousSupervised === undefined) {
    delete process.env.BYSPACE_SUPERVISED;
    return;
  }
...
test("openProject on a nonexistent directory does not broadcast an upsert that is immediately removed", async () => {
  ...
  const tempParent = await mkdtemp(path.join(os.tmpdir(), "byspace-open-project-"));
...
test("openProject expands tilde before creating the workspace", async () => {
  ...
  const workspacePath = await mkdtemp(path.join(home, ".byspace-open-project-"));
