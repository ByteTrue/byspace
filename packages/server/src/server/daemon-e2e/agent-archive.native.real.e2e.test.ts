...
async function expectContextAfterWorkspaceRestore(legacyNativeArchive: boolean): Promise<void> {
  const root = mkdtempSync(path.join(tmpdir(), "byspace-archive-codex-"));
  ...
  try {
    ...
    // Older daemons marked BySpace archived while native archive failed against its writer.
...
test.runIf(process.env.BYSPACE_NATIVE_ARCHIVE_QA === "1").each([false, true])(
