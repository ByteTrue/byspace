...
function seedSameCwdWorkspaces(): { byspaceHomeRoot: string; cwd: string } {
  const byspaceHomeRoot = mkdtempSync(path.join(tmpdir(), "byspace-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-same-cwd-dir-"));
  const projectsDir = path.join(byspaceHomeRoot, ".byspace", "projects");
  ...
  return { byspaceHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ byspaceHomeRoot: string; cwd: string }> {
  const byspaceHomeRoot = mkdtempSync(path.join(tmpdir(), "byspace-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-legacy-agent-dir-"));
  ...
  const agentStorage = new AgentStorage(path.join(byspaceHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { byspaceHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestBySpaceDaemon({ byspaceHomeRoot });
...
test("workspace.create directory source with firstAgentContext generates a daemon-visible workspace title", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-named-local-dir-"));
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-title-snapshot-storm-"));
...
test("create_agent_request with workspaceId does not retitle an existing workspace", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-agent-submit-title-"));
  const daemon = await createTestBySpaceDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("creating another same-cwd local workspace keeps running status on the owning workspace only", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "byspace-running-same-cwd-create-"));
  const daemon = await createTestBySpaceDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { byspaceHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestBySpaceDaemon({ byspaceHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
