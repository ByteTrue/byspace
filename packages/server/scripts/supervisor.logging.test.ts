    ...
    const result = await runSupervisorFixture({
      workerSource: `
        process.on("message", (message) => {
          if (message?.type === "byspace:graceful-shutdown") process.exit(0);
        });
  ...
  test("lets the worker clean up its descendant before supervised shutdown", async () => {
    const result = await runSupervisorFixture({
      workerSource: `
        ...
        process.on("message", (message) => {
          if (message?.type !== "byspace:graceful-shutdown") return;
        ...
        process.send?.({ type: "byspace:shutdown", reason: "descendant_cleanup_probe" });
        setInterval(() => {}, 1000);
      `,
    });
