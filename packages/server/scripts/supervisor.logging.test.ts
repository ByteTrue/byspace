...
describe("supervisor durable logging", () => {
  ...
  test("keeps a worker alive while heartbeats continue", async () => {
    const result = await runSupervisorFixture({
      ...
      workerSource: `
        setInterval(() => {
          process.send?.({ type: "byspace:worker-heartbeat" });
        }, 250);
        setTimeout(() => {
          process.send?.({ type: "byspace:shutdown", reason: "healthy_heartbeat_test_complete" });
        }, 6000);
      `,
    });
    ...
    const result = await runSupervisorFixture({
      ...
      workerSource: `
        ...
        if (!existsSync(marker)) {
          ...
          setTimeout(() => {
            process.send?.({ type: "byspace:shutdown", reason: "heartbeat_pause_tolerated" });
          }, 7_000);
          setInterval(() => {}, 1_000);
        } else {
          process.send?.({ type: "byspace:shutdown", reason: "unexpected_heartbeat_restart" });
          setInterval(() => {}, 1_000);
        }
      `,
    });
      ...
      const result = await runSupervisorFixture({
        ...
        workerSource: `
          ...
          if (!existsSync(marker)) {
            ...
            process.send?.({ type: "byspace:shutdown", reason: "watchdog_test_complete" });
            setInterval(() => {}, 1000);
          }
        `,
      });
