...
BYSPACE_HOME=~/.paseo        # Runtime state directory (agents/, etc.)
BYSPACE_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`BYSPACE_HOME` defaults to `~/.paseo` and isolates runtime artifacts like `agents/`. `BYSPACE_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
BYSPACE_HOME=~/.paseo-blue BYSPACE_LISTEN=127.0.0.1:7777 npm run dev
```
