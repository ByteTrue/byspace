...
function release({
  ...
  return {
    tag_name: tag,
    assets: hasApk ? [{ name: `byspace-${tag}-android.apk` }] : [],
    prerelease,
    draft: false,
  };
}
...
function desktopRelease({
  ...
  return {
    tag_name: `v${version}`,
    assets: [
      { name: `BySpace-${version}-arm64.dmg` },
      { name: "BySpace-x86_64.AppImage" },
      { name: `BySpace-Setup-${version}-x64.exe` },
      { name: `BySpace-Setup-${version}-arm64.exe` },
    ],
    prerelease,
    draft,
  };
...
describe("selectReleaseChannels", () => {
  it("offers the newest prerelease when it leads stable", () => {
    ...
    expect(channels.beta?.windowsArm64Asset).toBe("BySpace-Setup-0.3.0-beta.2-arm64.exe");
  });
