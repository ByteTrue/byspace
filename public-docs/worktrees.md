...
This page covers the git-specific details: where worktrees live, how branches are chosen, and how to configure setup hooks, scripts, terminals, and long-running services through `byspace.json`. Legacy `byspace.json` files remain supported.

## Layout and workflow

Worktrees live under `$BYSPACE_HOME/worktrees/` by default, grouped by a hash of the source checkout path. You can change the base directory with `worktrees.root` in `config.json`. Each worktree gets a slug and a branch when its workspace is created.
...
With a custom root, BySpace keeps the same hashed layout under that directory:
...
1. Create a workspace with worktree isolation; BySpace creates the worktree and runs your setup hooks
2. Launch one or more agents in that workspace
3. Review the diff against the base branch
4. Merge or archive the workspace; after the last workspace using it is archived, BySpace runs teardown and removes the worktree
...
Add `--forge <name>` when BySpace cannot infer the forge from the source checkout.
...
`scripts` are named commands you can run inside a worktree on demand. Mark one as a _service_ and BySpace supervises it as a long-running process, assigns it a port, and routes HTTP traffic to it through the daemon's reverse proxy.

Run them from the app, or manage them from automation with [`byspace script`](/docs/cli#workspace-scripts) and the [workspace-script MCP tools](/docs/mcp#workspace-scripts).
...
Omit `port` to let BySpace auto-assign one. Bind your process to `$BYSPACE_PORT` rather than hard-coding; each worktree gets a distinct port so multiple copies of the same service coexist.

### Dynamic port allocation

By default, BySpace asks the OS for an available ephemeral port. Configure a range globally in
`~/.byspace/config.json` or per project in `byspace.json`:
...
BySpace runs the executable in the workspace directory with four arguments: service name, workspace
...
are available as `BYSPACE_SCRIPTNAME`, `BYSPACE_WORKSPACE_ID`, `BYSPACE_BRANCH_NAME`, and
`BYSPACE_WORKTREE_PATH`. It must print one valid TCP port to stdout. `portScript` wins over `range` in
the same block. BySpace trusts the external allocator, so the returned port may already be in use, for
example by a service BySpace will attach to.
...
BYSPACE_URL=http://web--my-app.localhost:6777  # this service's proxy URL
BYSPACE_SERVICE_API_PORT=51732
BYSPACE_SERVICE_API_URL=http://api--my-app.localhost:6777
BYSPACE_SERVICE_WEB_PORT=3000
BYSPACE_SERVICE_WEB_URL=http://web--my-app.localhost:6777
```
...
byspace run --workspace <workspace-id> "implement auth"
byspace workspace rename <workspace-id> "Auth rework"
byspace workspace archive <workspace-id>
```

For the common case, `byspace run --new-workspace worktree --worktree-mode branch-off --new-branch feature/auth --base origin/main "implement auth"` creates both the workspace and its first agent.
