...
description: Serve the BySpace web app from your own daemon and reach it over your own LAN, VPN, reverse proxy, or tunnel.
nav: Web UI
order: 6
...
BySpace's daemon can serve the browser web app itself, from the same address it already uses for the API. You don't need the hosted app at [app.byspace.cc.cd](https://app.byspace.cc.cd): point a browser at your own daemon and you get the full UI, connected to your own agents, on infrastructure you control.
...
server {
  ...
  ssl_certificate     /etc/letsencrypt/live/byspace.example.com/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/byspace.example.com/privkey.pem;
...
For the remote/relay path (driving a daemon through the BySpace relay rather than a reverse proxy), the relay has its own public-vs-internal TLS settings, see [Security](/docs/security).
...
For the full threat model, relay encryption, and DNS-rebinding details, see [Security](/docs/security) and [SECURITY.md](https://github.com/ByteTrue/byspace/blob/main/SECURITY.md).
