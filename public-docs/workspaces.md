---
title: Workspaces
description: Understand how BySpace groups working directories, agents, terminals, and browsers into workspaces.
nav: Workspaces
order: 10
category: Workspaces
---
...
BySpace is organized around workspaces, not chats.
...
The sidebar starts with projects. A project can be a git repository, a GitHub project, or any directory on a machine running the BySpace daemon.
...
The workspace is the product concept; a git worktree is one way to isolate its files. More than one workspace can refer to the same managed worktree, and BySpace removes that worktree after its last workspace is archived.
...
byspace workspace create --isolation worktree --path ~/dev/my-app --base origin/main
```
...
Creating an agent and creating a workspace are separate actions. Pass a workspace ID when you want an agent in a specific existing workspace. A bare `byspace run` from a human shell creates a new local workspace; when one agent runs it, BySpace recognizes the caller and creates a subagent in the caller's workspace.

## Worktrees

Every workspace in BySpace is backed by a working directory. When that directory is a git worktree, you get a separate branch and isolated environment for each task.
