...
This prompts for a password, writes the bcrypt hash to `config.json`, and tells you to restart the daemon. Authentication is a startup setting, so `byspace reload` will also report it as restart-required.
...
New homes write `daemon.relay.enabled: false`. BySpace asks before enabling relay when you pair a device; existing homes keep their saved value. See [Connectivity](/docs/connectivity) to choose and configure a connection method, and [Security](/docs/security) for the relay encryption model.
...
`BYSPACE_RELAY_ENABLED=true|false` overrides the persisted value for that daemon launch. The matching `byspace daemon start --relay` and `--no-relay` flags have the same authority. Remove the launch override before changing relay from BySpace Desktop or `byspace daemon pair --relay`.
...
Legacy `BYSPACE_*` names remain accepted as lower-priority compatibility aliases.
