...
List, start, and stop the scripts configured in a workspace's `byspace.json`:
...
`byspace plugin logs <id>` returns the plugin's recent daemon-side stdout and stderr. Add `--json` for
...
Run deploy from the project root. It reads `.byspace/hub.yml`, every direct `.byspace/workflows/*.yml` file, and referenced `.byspace/workflows/partials/*` files in deterministic path order. It does not search parents, accept an alternate resource path, or flatten the bundle into monolithic YAML.
