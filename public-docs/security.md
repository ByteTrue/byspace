...
`--api-key` style flags and matching environment variables override stored credentials without being persisted. Prefer environment or secret-manager injection for automation, and avoid command-line flags when local process listings or shell history are visible to other users.
