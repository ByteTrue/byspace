...
`--api-key` and `BYSPACE_HUB_API_KEY` (or legacy `PASEO_HUB_API_KEY`) override stored login without being persisted. Prefer environment or secret-manager injection for automation, and avoid command-line flags when local process listings or shell history are visible to other users.
