...
```sh
BYSPACE_HUB_URL=https://hub.example.com BYSPACE_HUB_API_KEY=byspace_pk_... byspace hub connect
```

Origin precedence is explicit `[origin]`, `BYSPACE_HUB_URL`, active stored login, then `https://hub.byspace.cc.cd`. An explicit `--api-key <secret>` takes precedence over the environment and an exact-origin stored login.
...
`${{ byspace.execution.id }}` renders the execution's UUID, so every execution gets its own branch off `origin/main`.
