...
description: Local plugin files, contributions, BySpace SDK access, RPCs, attachments, logs, lifecycle, hosts, and CLI commands.
nav: Reference
order: 46
category: Plugins
---
...
- React Native Web surfaces and sidebar items to BySpace browser clients;
...
The generated declaration file supplies `@bytetrue/byspace/plugin` and `@bytetrue/byspace/plugin/server` types for local typechecking. BySpace supplies the runtime modules. Regenerate a fresh project with the matching CLI when the plugin contract changes.
...
BySpace rejects imports from `*.server` files into client modules and imports from `*.client` files into server modules. Keep shared modules free of Node and React Native runtime code.
...
```tsx
import type { PluginSurfaceProps } from "@bytetrue/byspace/plugin";
...
| `layout` | `compact` and the `web` platform.                              |

BySpace owns the route, header, close action, host picker, error boundary, and query client. The plugin owns the surface body. Icons use [Lucide](https://lucide.dev/icons/) names.

## Theme and layout

Plugin UI runs in the BySpace browser Web app, across every BySpace theme. `theme` is a typed `PluginTheme`. Color and spacing must come from those props. Unstyled `Text` is black and fails in dark themes.
...
| `layout.compact`                | Padding and stacking       | `true` on narrow browser windows    |
| `layout.platform`               | Platform-specific behavior | `web`                               |
...
Client code can import `react`, `react-native`, `@tanstack/react-query`, `zod`, `@bytetrue/byspace/plugin`, and `@bytetrue/byspace/plugin/server`. Install them locally for typechecking; BySpace provides the client runtime instances.
...
```tsx
import { type PluginAgentPanelProps, useAgent, useWorkspace } from "@bytetrue/byspace/plugin";
import { useMemo } from "react";
import { Text, View } from "react-native";
...
| `byspace`              | All                 | Selected host's existing `BySpaceApi`.                        |
...
Use `useBySpace()` for ordinary BySpace operations from a surface. It borrows the selected host's existing connection; do not create another client.
...
The returned API covers workspaces, agents, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because BySpace owns the connection.
...
Backend handlers receive the same `BySpaceApi` as `{ byspace }`. Their connection belongs to the subprocess and closes when the plugin stops. Backend code can use Node APIs and dependencies installed in the plugin directory.
