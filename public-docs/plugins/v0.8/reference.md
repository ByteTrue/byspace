...
returns an idempotent remover. The entry cleanup runs before BySpace removes remaining registrations.
...
`byspace`, typed `rpc`, `openSurface`, explicit-context `openPanel`, and every client registration.
`addComposerPill` returns an idempotent removal function. BySpace also removes every outstanding pill
when the plugin installation or host connection is torn down.

BySpace owns the pressable, shared pill chrome, pending state, error reporting, and track-bar
