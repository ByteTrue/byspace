...
> **For BySpace v0.8 beta.** Return to the [v0.8 quickstart](/docs/plugins/v0.8).

Migrating an existing plugin? Follow the standalone [runtime-entry migration guide](/docs/plugins/v0.8/migration).

Local plugins are directory sources installed into one BySpace daemon. A plugin can contribute:

- React Native surfaces and sidebar items to BySpace clients;
...
- normal BySpace operations through the TypeScript SDK;
- searchable external resources in the message composer.

Plugin code is trusted and unsandboxed. Client surfaces run in the BySpace app. Backend contributions run in a subprocess with access to the daemon machine, including its files, processes, credentials, and network.

## Project files

`byspace plugin init /absolute/path/to/my-plugin` creates:
...
The required root manifest is `paseo-plugin.json`. It contains the default plugin ID and supported BySpace versions:
...
`requirements` is an optional object. Its currently supported key, `byspace`, accepts an npm semver
range. An omitted `requirements.paseo` means `<0.8.0`: the plugin predates the first breaking
plugin release. BySpace 0.8 and later reject it with a link to the [migration guide](migration).
...
Prerelease BySpace versions also satisfy a range their stable core (`major.minor.patch`) satisfies, so `0.8.0-beta.1` satisfies `>=0.8.0` but not `<0.8.0`.

`byspace plugin init` writes `>=` followed by the current CLI version and pins the matching SDK
...
For example: `Plugin "review" requires BySpace >=0.8.0. Your daemon is 0.7.2.` Use a compatible plugin
...
| ------------------ | ----------------------- | --------------------- | ------------------------------------------------------------------------------- |
| `index.client.tsx` | BySpace app, per client | `PluginClientContext` | When the plugin has any UI, callback, theme, or attachment source               |
...
BySpace builds each bundle from its matching entry. An import from `client/` into the daemon bundle,
...
BySpace provides these modules to client code:
...
| `@getpaseo/plugin/client/react-native` | BySpace UI components and UI hooks                                                                |
...
Do not import `lucide-react-native`, `react-native-svg`, or DOM libraries. Set contribution `icon` fields to a [Lucide icon name](https://lucide.dev/icons/); BySpace validates the name and renders the icon.
...
BySpace provides `@getpaseo/plugin`, `@getpaseo/plugin/server`,
...
Provider settings are toggle/select descriptors that BySpace renders in the composer. Keep
provider-private JSON under `providerOptions`. Host tools arrive as MCP servers in the complete
session config.

BySpace refreshes an agent by closing its current provider session and opening it with current
...
which return `{ update, remove }` handles. The entry cleanup runs before BySpace removes remaining registrations.
...
Cleanup can be async. Release timers, watchers, sockets, and other resources created by the plugin. BySpace also removes registrations, unmounts surfaces, rejects pending RPCs, closes the plugin's daemon session, and stops its subprocess on reload, disable, removal, disconnect, or daemon shutdown.
...
| ------------------------------- | ------------------------------------ | ------------------------------------------------------------ |
| `server.on(name, callback)`     | `(event, { byspace, signal })`       | `void` or `Promise<void>`                                    |
| `server.before(name, callback)` | `({ request }, { byspace, signal })` | Modified request, or `undefined` to keep it; async supported |
...
Copy [server/inspect.ts](https://github.com/ByteTrue/byspace/blob/main/plugin-examples/lifecycle-actions/server/inspect.ts)
...
Using `shellCommand` from the same [helper file](https://github.com/ByteTrue/byspace/blob/main/plugin-examples/lifecycle-actions/server/inspect.ts):
...
Creation request
  ...
  → derive launch configuration with BySpace runtime tools and daemon prompt
...
| Plugin                                                                                                   | Includes                                                                     |
| -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| [lifecycle-logger](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/lifecycle-logger)       | All eleven hooks; JSON logs with environment values redacted                 |
| [lifecycle-actions](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/lifecycle-actions)     | Follow-ups, permissions, environment, provider switching, worktree selection |
| [agent-configuration](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/agent-configuration) | MCP injection and Codex sandbox/approval options                             |

Read logger output with `byspace plugin logs lifecycle-logger` or the host's `daemon.log`.
...
BySpace modal. They accept React Native props and refs and integrate with the sheet's gestures. Outside
...
The runnable [modal UI example](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/modal-ui)
...
`Icon` renders a [Lucide icon](https://lucide.dev/icons/) from BySpace's installed icon set. Plugin bundles do not import
...
registrations are client contributions. BySpace applies the transformer while building the render
...
Each replacement may set an optional plugin-local `id`; otherwise BySpace uses its index within that
source item's output.

Renderers receive `agentId`, `item`, `timestamp`, `theme`, `host`, and `layout`. BySpace validates
`item.data` with the registered schema before rendering. Keep transformers synchronous and
deterministic. BySpace memoizes results by source-item reference and derives replacement identity from
the source row, so updates to one streaming item do not remount its renderer. Use the exported
`useRevealedText(text, phase)` hook when a renderer should pace streaming text like BySpace's built-in
assistant rows.
...
same plugin and `id`. If its renderer is missing, BySpace shows the existing unavailable row. Daemons
...
Every color is a hex string; anything else fails to load. BySpace expands the palette into the full
...
`appearance` is `"light"` or `"dark"`. BySpace uses it to select the matching surface, status,
...
later disabled or removed, BySpace falls back to the default theme rather than leaving the app
unpainted.
...
The component receives `PluginSurfaceProps`. BySpace owns the header, back navigation, safe areas,
...
| `SettingsSelect`                   | Row props plus required string `value`, `options: { label, value }[]`, `onValueChange`; optional `disabled`. Uses BySpace's adaptive menu. |
...
See the complete [settings example](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/settings)
...
Read cached state with `useWorkspace(workspaceId, selector)` and `useAgent(agentId, selector)`. A selector is required. BySpace compares its result shallowly, so selecting `{ name, status }` does not re-render when unrelated fields change. Select every field the component renders in one call; do not select the whole snapshot.
...
BySpace owns tab focus, splitting, closing, persistence, query state, the API/RPC providers, and the render error boundary. A restored tab whose plugin, panel, context, workspace, or agent is unavailable stays open with an unavailable message instead of crashing the workspace.
...
client.addCommandCenterItem({
  ...
  keywords: ["inspect"],
  context: "agent",
  async onSelect({ byspace, rpc, workspace, agent, openPanel }) {
    await byspace.workspaces.ref(workspace.id).setTitle(`Review ${agent.id}`);
...
| `byspace`                 | All                 | Selected host's existing `PaseoApi`.                                                                            |
...
An agent callback may open either an agent panel or a workspace panel. A workspace callback may open only a workspace panel. Unknown surface and panel IDs fail visibly. Use `byspace` for normal workspace, agent, provider, and daemon-config operations. Use `rpc` for plugin-specific filesystem, credential, vendor, or daemon-local work.

## Slash commands

Register a command that runs in the BySpace client when the user submits `/name args` from the
...
`args` is `"src"`; BySpace trims only the remainder's leading and trailing whitespace and leaves
parsing to the plugin. BySpace owns the autocomplete row, input clearing, and the error toast. It
...
Try the [button example](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/buttons) for
...
Compact header buttons use icons without labels or chevrons. BySpace moves excess contributions
...
An action runs on the client. BySpace marks the button busy until its promise settles, blocks repeated
presses, and shows failures in a toast. A failed action can be retried. Use the client's `byspace` for
...
BySpace removes leading, trailing, and consecutive separators after filtering hidden items.
...
Render a React Native icon or indicator within the supplied size. BySpace bounds the icon slot and
...
Render the body only; BySpace owns anchoring, scrolling, padding, and sheet presentation. Content can
...
`remove()` is idempotent. Updates after removal do nothing. BySpace removes outstanding buttons when
...
Use `usePaseo()` for ordinary BySpace operations from a surface. It borrows the selected host's existing connection; do not create another client.
...
The returned API covers projects, workspaces, agents, terminals, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because BySpace owns the connection.

## Add plugin-specific backend behavior

Use plugin RPC only for work that is not a normal BySpace operation: reading a vendor API, accessing daemon-local resources, or keeping credentials off the client.
...
BySpace adds `[byspace]` entries when the plugin starts loading, becomes ready, starts stopping, and has
stopped. It records compilation and load failures as stderr entries, including failures that happen
before the plugin subprocess starts. BySpace also captures output emitted during initialization, RPC
...
byspace --host <url> plugin logs my-plugin
```
...
BySpace retains up to 500 entries and 256 KiB per plugin in memory. Individual lines are capped at
...
BySpace owns the composer menu, search picker, selected pill, draft state, and submission. The `text` value is the complete snapshot sent to the agent.

## Hosts and lifecycle

Plugins are installed per daemon. When the same contribution exists on several connected hosts, BySpace shows one sidebar item and adds a host picker. The selected host supplies the bundle, BySpace API, RPC transport, and query cache. Calls never fall through to another host when the selected host is offline.
...
Use `update` when you want BySpace to contact a tracked Git remote and install an available update.
...
> **Trust every plugin you add.** `byspace plugin add` and `byspace plugin install` mean “I trust this codebase.” Server code and Git preparation commands run unsandboxed with the daemon user's access on the daemon host; client contributions run inside BySpace. Dependencies and future updates are part of that decision. With the global `--host` option, commands run on the remote daemon host.
...
| Edited code does not appear                                           | Run `npm run typecheck`, then `byspace plugin reload <id>`.                                                                               |
| Reload fails                                                          | Read `byspace plugin ls` and `byspace plugin logs <id>`, fix the source error, then reload; BySpace does not restore the previous bundle. |
| Plugin exits unexpectedly                                             | Read `byspace plugin logs <id>` for retained initialization, cleanup, stderr, and final crash output.                                     |
