...
description: Add a coding agent to BySpace directly or adapt an ACP agent.
...
> **For BySpace v0.8 beta.** Start with the
> [plugin quickstart](/docs/plugins/v0.8) if you have not built a BySpace plugin before.

A provider plugin connects a coding agent to BySpace without adding it to BySpace core. Publish the
plugin in a Git repository and users can install and update it with `byspace plugin add` and
`byspace plugin update`.
...
- [`provider-direct`](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/provider-direct): sessions, settings, prompts, persistence, child sessions, and a provider-owned timeline renderer;
- [`provider-acp-transformer`](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/provider-acp-transformer): an ACP command template with a Zod-validated vendor edit transformer. Replace `example-acp --stdio` with an installed ACP agent before loading it;
- [`inline-thinking`](https://github.com/ByteTrue/byspace/tree/main/plugin-examples/inline-thinking): a renderer-only plugin that does not implement a provider.
...
BySpace requests the catalog before creating a session. Return the choices needed by the agent form:
...
the user timeline item so BySpace replaces its optimistic message. Every started turn needs one
...
BySpace renders these controls in the composer. A user change arrives as `session.configure`; apply it
...
for provider-specific configuration that BySpace does not render. A provider-specific token budget
...
Return an opaque persistence value in `session.opened` or `session.persistence`; BySpace stores it
...
BySpace refreshes an agent by closing its current provider session and opening it again with current
...
Use `restoration: "core"` when BySpace can reopen a session from persistence. A provider-owned child
...
Advertise only capabilities you implement. If steering is unsupported, omit `prompt.steer`; BySpace
...
byspace plugin update my-provider-plugin
```

Keep vendor compatibility and releases in that repository. BySpace core should only change when the
