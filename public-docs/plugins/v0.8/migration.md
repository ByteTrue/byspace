...
> **For BySpace v0.8 beta.** This migration is not required for BySpace v0.7.
...
8. Delete the old root entry. BySpace does not load it.
...
`@getpaseo/plugin/server`. Remove imports of the old context type. Client registrations return idempotent removal functions, except header buttons and composer pills, which return `{ update, remove }` handles. Preserve any remover the plugin calls before teardown; BySpace removes outstandingregistrations after the entry cleanup runs.
...
BySpace's app integration and is never a plugin-author import.
...
| `This plugin was made for an older version of BySpace`                                                                     | The directory still has only the old root entry. Create a runtime entry, move registrations, then delete the old file.    |
...
## 7. Declare the BySpace requirement
...
Keep your existing ID and build commands. Missing `requirements.paseo` means `<0.8.0`, so BySpace 0.8
...
```bash
npm run typecheck
byspace plugin reload <plugin-id>
byspace plugin ls
```
