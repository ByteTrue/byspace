...
> **For BySpace v0.8 beta.** Use the [v0.7 docs](/docs/plugins/v0.7)
> if you run the stable release.

> **Experimental:** The plugin API is still evolving, so expect breaking changes and updates to
> your plugins as BySpace evolves. See the [plugin roadmap](https://github.com/ByteTrue/byspace/labels/plugins)
> for planned contribution surfaces.

A plugin is a TypeScript project installed into one BySpace daemon. It can add
...
contributions run on every BySpace client connected to that daemon, including mobile.
...
development dependencies for typechecking and tests only; BySpace supplies the plugin SDK, React,
...
```text
workspace-plugin/
  paseo-plugin.json      # plugin ID and supported BySpace versions
  index.client.tsx       # runs in the BySpace app
...
user's access on the daemon machine, and client code runs inside the BySpace app. Installing a plugin
...
daemon's `config.json`; after editing the file, apply it with `byspace reload --json`. An automated
...
`byspace plugin ls` should report the plugin as `running`. Open BySpace, choose **Greeting** in the
...
the client is viewing the host you installed into. `byspace plugin logs workspace-plugin` shows the
...
`theme.colors`, and `layout.compact` drives spacing, so the panel works in every BySpace theme and on
...
A failed reload stays failed and reports its error in `byspace plugin ls`; fix the source and reload
again.
...
BySpace compiles TypeScript itself, so most plugins need no build step. A repository that must
install a dependency BySpace does not provide, or generate files, declares
...
The tail includes `[byspace]` loading, ready, stopping, and stopped entries, plus compilation and load
...
- [TypeScript SDK](/docs/sdk): the workspace, agent, provider, and config API available as `byspace`
