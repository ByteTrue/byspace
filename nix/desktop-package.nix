...
buildNpmPackage {
  ...
  buildPhase = ''
    ...
    npm run build --workspace=@bytetrue/byspace-expo-two-way-audio

    # Expo web export for the Electron renderer
    ( cd packages/app && BYSPACE_WEB_PLATFORM=electron npx expo export --platform web )
  ...
  installPhase = ''
    ...
    ${lib.optionalString stdenv.hostPlatform.isLinux ''
      ...
      BYSPACE_TRACE_DESKTOP=1 node scripts/trace-daemon.mjs > desktop-files.txt
  ...
  desktopItems = lib.optionals stdenv.hostPlatform.isLinux [
    ...
    # package.json `name` ("byspace-desktop"), 38 uses the runtime app name that
    # main.ts sets ("BySpace"). Ship a NoDisplay entry for the second spelling so
