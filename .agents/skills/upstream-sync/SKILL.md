...
- Do not redo BySpace identity migration or flatten maintained platform boundaries. Browser Web/PWA, published Android and Electron Desktop, maintained source-only iOS, Desktop Browser automation, and all tests/build/package/release support remain retained surfaces. Preserve the same-tag Desktop/Android release gate and do not activate iOS CD.
...
5. Identify only explicitly documented excluded-product changes and their cross-layer dependencies. Never classify Android, iOS, Electron, Browser automation, or platform release support as excluded because of distribution status. Android/Desktop publication is active; iOS implementation is maintained while executable iOS CD is intentionally absent. Give every behavior an internal disposition and prove complete commit/file coverage.
