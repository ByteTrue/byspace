...
Update `ByteTrue/byspace` with a complete Paseo release using standard Git. Keep the process small: no custom sync engine, state file, helper script, release, or deployment.
...
- Merge the complete target with standard Git and preserve its ancestry. Never copy a release tree, cherry-pick selected upstream features, squash upstream history, or rebase/force-push `main`.
...
- PR creation does not authorize merge. Never enable auto-merge. Merge only after a second explicit user instruction, and use a merge commit—not squash or rebase—so upstream ancestry reaches `main`.
...
4. Compare candidates with the upstream release commit already reachable from current `main`. Do not assume a release commit belongs to upstream `main`.
...
5. If the target commit is already an ancestor of frozen `origin/main`, report `already-synced` and stop without creating a branch or candidate.
6. Find `LAST_UPSTREAM_SHA`: the most recent previously accepted Paseo release commit reachable from the frozen main, using prior sync merge ancestry and namespaced release refs. If it is missing or ambiguous, show the candidates and ask the user; do not guess.
...
Do not create a database or metadata file for it. Recover it from refs, merge ancestry, and the conversation when continuing; stop if recovery is ambiguous.
...
1. In the sync worktree, merge the complete frozen target with `--no-ff --no-commit` so the result can be reviewed before committing.
...
8. Commit the merge with both parents and a message such as `chore(sync): merge Paseo vX.Y.Z`. Verify:

   ```bash
   git merge-base --is-ancestor "$UPSTREAM_TARGET_SHA" HEAD
   ```
...
3. Verify the candidate is clean and the upstream target is its ancestor.
...
## `merge`: preserve ancestry in main
...
4. Fetch `origin/main` and verify the frozen upstream target is now its ancestor:

   ```bash
   git merge-base --is-ancestor "$UPSTREAM_TARGET_SHA" origin/main
   ```
