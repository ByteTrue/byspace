<p align="center">
  <img src="packages/website/public/logo.svg" width="64" height="64" alt="BySpace 로고">
</p>
...
[byspace.cc.cd/download](https://byspace.cc.cd/download) 또는 [GitHub 릴리스 페이지](https://github.com/ByteTrue/byspace/releases)에서 다운로드하세요. 앱을 열면 데몬이 자동으로 시작됩니다. 별도로 설치할 것은 없습니다.

휴대폰에서 연결하려면 **설정 → 호스트 → 기기 페어링**을 여세요.

### CLI / 헤드리스 환경

CLI를 설치하고 BySpace를 시작하세요.
...
BySpace가 로컬에서 시작된 뒤 기기 페어링을 위한 종단 간 암호화 릴레이를 켤지 묻습니다. 거절하면 TCP, Tailscale 또는 다른 VPN으로 직접 연결할 수 있습니다. 이 방식은 서버나 원격 머신에서 유용합니다.

자세한 설치와 설정은 아래 문서를 참고하세요.

- [문서](https://byspace.cc.cd/docs)
- [연결 가이드](https://byspace.cc.cd/docs/connectivity)
- [설정 레퍼런스](https://byspace.cc.cd/docs/configuration)
...
byspace ls                           # 실행 중인 에이전트 목록
byspace attach abc123                # 실시간 출력 스트리밍
byspace send abc123 "also add tests" # 후속 작업 전송

# 원격 데몬에서 실행
byspace --host workstation.local:6777 run "run the full test suite"
```

자세한 내용은 [전체 CLI 레퍼런스](https://byspace.cc.cd/docs/cli)를 참고하세요.

## 스킬

스킬은 에이전트가 BySpace를 통해 다른 에이전트를 오케스트레이션하는 방법을 알려 줍니다.
...
- `packages/website`: 마케팅 사이트 및 문서(`byspace.cc.cd`)
...
- [ByteTrue/byspace-relay](https://github.com/ByteTrue/byspace-relay) — Elixir로 작성한 공식 분산형 릴레이
- [byspace-skins](https://github.com/huangguang1999/byspace-skins) — 커뮤니티 테마와 Agent Skill을 제공하고, 코드 수정 없이 쓸 수 있는 데스크톱 테마 로더
- [byspace-vscode](https://marketplace.visualstudio.com/items?itemName=hinnes.byspace-vscode) — VS Code 확장 프로그램
