...
This is ByteTrue's first self-built baseline from BySpace `v0.7.0-beta.2`. It keeps the upstream feature set while isolating BySpace identity and local runtime state.
...
BySpace uses `~/.byspace`, `BYSPACE_HOME`, `BYSPACE_LISTEN`, port `6777` in production, and port `6778` in the development checkout. It does not reuse BySpace's state directory or default ports.
