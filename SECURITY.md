...
2. The pairing URL (rendered as a QR code or opened directly) carries the daemon's public key in its URL fragment (`https://app.byspace.cc.cd/#offer=...` for Stable or `https://app-beta.byspace.cc.cd/#offer=...` for Beta). Fragments are not sent to the web server, so the channel's Pages site never sees the key.
