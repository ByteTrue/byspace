...
BySpace validates the `Host` header on every HTTP request and every WebSocket upgrade against an allowlist (Vite-style semantics). By default, only `localhost`, `*.localhost`, and any literal IP address (IPv4 or IPv6) are accepted. Additional hostnames can be configured via `hostnames` in `config.json` or the `BYSPACE_HOSTNAMES` env var (legacy `PASEO_HOSTNAMES` remains accepted). Entries beginning with `.` match a domain and its subdomains; the value `true` disables the allowlist entirely. Requests with unrecognized hosts are rejected with `403 Host not allowed`.
