...
2. The pairing URL (rendered as a QR code or opened directly) carries the daemon's public key in its URL fragment (`https://byspace.pages.dev/#offer=...`). Fragments are not sent to the web server, so `byspace.pages.dev` never sees the key.
