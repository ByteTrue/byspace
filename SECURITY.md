...
2. The pairing URL (rendered as a QR code or opened directly) carries the daemon's public key in its URL fragment (`https://byspace.pages.dev/#offer=...` for Stable or `https://byspace-beta.pages.dev/#offer=...` for Beta). Fragments are not sent to the web server, so the channel's Pages site never sees the key.
