...
2. The pairing URL (rendered as a QR code or opened directly) carries the daemon's public key in its URL fragment (`https://app.byspace.cc.cd/#offer=...`). Fragments are not sent to the web server, so `app.byspace.cc.cd` never sees the key.
...
When BySpace checks out a change request from a different repository, it does not run that workspace's `byspace.json` setup, automatic terminals, named scripts, or teardown until you explicitly run setup for that workspace. The decision lasts for the workspace and does not re-prompt after new commits. Same-repository changes, ordinary branches, local workspaces, agent launches, terminals, explicit shell commands, and metadata-generation instructions are outside this gate.
...
BySpace validates the `Host` header on every HTTP request and every WebSocket upgrade against an allowlist (Vite-style semantics). By default, only `localhost`, `*.localhost`, and any literal IP address (IPv4 or IPv6) are accepted. Additional hostnames can be configured via `hostnames` in `config.json` or the `BYSPACE_HOSTNAMES` env var (legacy `BYSPACE_HOSTNAMES` remains accepted). Entries beginning with `.` match a domain and its subdomains; the value `true` disables the allowlist entirely. Requests with unrecognized hosts are rejected with `403 Host not allowed`.
...
If you discover a security vulnerability, report it privately through [GitHub Security Advisories](https://github.com/ByteTrue/byspace/security/advisories/new). Do not open a public issue.
