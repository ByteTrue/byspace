...
resolve_dev_daemon_endpoint() {
  local endpoint="${BYSPACE_DEV_DAEMON_ENDPOINT:-}"
  if [ -n "$endpoint" ]; then
    echo "$endpoint"
    return
  fi
...
configure_dev_byspace_home() {
  local seed_home="${BYSPACE_DEV_SEED_HOME:-}"
  local managed_home="${BYSPACE_DEV_MANAGED_HOME:-0}"
  ...
  if [ -n "${BYSPACE_HOME:-}" ]; then
    ...
    mkdir -p "$BYSPACE_HOME"
    if [ "$managed_home" = "1" ] || [ -n "$seed_home" ]; then
      configure_dev_daemon_config
    fi
    return
  fi
...
configure_dev_command_env() {
  if [ -z "${BYSPACE_LISTEN:-}" ]; then
    local service_port="${BYSPACE_SERVICE_DAEMON_PORT:-}"
