...
import { mergeUpdaterManifest } from "./merge-updater-manifest.mjs";

test("preserves unknown fields while merging files by url", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "byspace-merge-updater-manifest-"));
  try {
    ...
    mergeUpdaterManifest(arm64Path, x64Path, outputPath);
