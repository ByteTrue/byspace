...
const preferredSimulatorType = process.env.BYSPACE_IOS_DEVICE_TYPE || "iPhone 16 Pro";
const byspacePort = requiredEnv("BYSPACE_PORT");
const worktreePath = process.env.BYSPACE_WORKTREE_PATH || rootDir;
const worktreeName = process.env.BYSPACE_BRANCH_NAME || basename(worktreePath);
const worktreeHash = createHash("sha1").update(worktreePath).digest("hex").slice(0, 8);
const simulatorName =
  process.env.BYSPACE_IOS_SIMULATOR_NAME || `BySpace ${worktreeName} ${worktreeHash}`;
const daemonEndpoint =
  process.env.BYSPACE_DEV_DAEMON_ENDPOINT ||
  `localhost:${process.env.BYSPACE_SERVICE_DAEMON_PORT || "6768"}`;
...
async function main() {
  ...
  console.log(`iOS preview: ${process.env.BYSPACE_URL || `http://127.0.0.1:${byspacePort}`}/.sim`);
