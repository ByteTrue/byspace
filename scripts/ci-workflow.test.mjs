...
test("focused contracts stay inside existing required checks", () => {
  ...
  assert.match(server, /npm run test --workspace=@byspace\/server/);
  ...
  assert.match(releasePackage, /registry\.byspace\.invalid/);
  assert.match(releasePackage, /node_modules\/\.bin\/byspace.*--version/);
  assert.match(releasePackage, /test ! -e .*node_modules\/\.bin\/byspace/);
