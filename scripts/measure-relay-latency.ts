...
const DIRECT_ENDPOINT = "localhost:6777";
const PING_COUNT = 20;
...
async function main() {
  ...
  await measurePings("Direct (localhost:6777)", directClient, PING_COUNT, WARMUP_COUNT);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const ws = new WebSocket(
      `wss://byspace-relay.bytetrue.workers.dev/ws?serverId=latency_probe_${Date.now()}&role=client&clientId=probe_${i}`,
    );
    await new Promise<void>((resolve, reject) => {
      ws.on("open", () => {
