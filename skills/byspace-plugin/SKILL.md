---
name: byspace-plugin
description: Build and manage trusted local BySpace plugins. Use when the user asks to create, edit, install, reload, enable, disable, remove, or troubleshoot a BySpace plugin; add lifecycle hooks; transform agent configuration, environment, MCP servers, or workspace creation; automate permissions or turn follow-ups; add a native surface, sidebar item, or workspace panel; add Command Center items or slash commands; add composer pills or attachment sources; transform, render, or append agent timeline items; contribute a theme; use BySpace from plugin code; or add plugin RPCs.
---
...
- [Plugin reference](https://byspace.sh/docs/plugins/v0.8/reference.md) ([browser page](https://byspace.sh/docs/plugins/v0.8/reference))
...
In the BySpace repository, use `public-docs/plugins/v0.8/reference.md` for the checkout's API, including
...
```tsx
// index.client.tsx
import type { PluginClientContext } from "@byspace/plugin/client";
...
```ts
// index.server.ts
import type { PluginServerContext } from "@byspace/plugin/server";
...
created by plugin code. Every client `add*` method returns an idempotent remover. BySpace calls the
...
```tsx
import type { PluginClientContext, PluginSurfaceProps } from "@byspace/plugin/client";
...
`@byspace/plugin/client`, server contexts to `/server`, and host UI to `/client/react-native` or `/client/ui`.
...
```tsx
import { useBySpace } from "@byspace/plugin/client";
...
```ts
// shared/greeting.ts
import { defineRpc } from "@byspace/plugin";
import { z } from "zod";
...
```ts
// server/greeting.ts
import type { RpcInput } from "@byspace/plugin";
...
```tsx
// client/greeting.tsx
import { useRpc } from "@byspace/plugin/client";
import { greeting } from "../shared/greeting";
...
```ts
// shared/issues.ts
import { defineAttachmentSource, defineRpc } from "@byspace/plugin";
import { z } from "zod";
...
byspace plugin add owner/repository              # Git source; append :path for a monorepo subdirectory
byspace plugin add owner/repository --ref main   # branches track, tags and commits pin
