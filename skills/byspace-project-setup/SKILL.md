...
Implement only approved recommendations. Before the first write, record the exact files you will change and each file's current contents or absence so this invocation can be rolled back without touching unrelated work.
...
- make every edit idempotent; repeating the same approved setup must not duplicate scripts, fields, or commands
...
If an approved edit or its required validation fails, restore only this invocation's changes from the recorded state. Never overwrite a file that changed concurrently; stop and report the affected file and manual recovery path instead.
