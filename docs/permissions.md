...
A principal is the durable identity the daemon authorizes. A credential proves that a device or service represents it. Keep them separate so you can rotate credentials, attach more than one device, and revoke a linked service without inventing daemon user accounts.
...
`hub.execute` remains in the permission set only for wire compatibility with old clients; the
Hub RPC surface it gated is retired and every `hub.*` request receives an error response.
