...
Projects use `byspace.json`. There is no fallback filename and no conflict case: a project has at
most one config file.
...
`byspace reload` reads and validates this file once inside the daemon. That snapshot drives resolution,
...
BySpace writes it atomically. An update creates and validates a new version directory before changing
...
| -------------------------------------- | ------------------------ |
| `BYSPACE_GIT_MAX_PROCESSES_PER_SECOND` | `maxProcessesPerSecond`  |
...
**Path:** `$BYSPACE_HOME/projects/workspace-labels.json`
...
| Path                  | Format                                                         | Notes                                                                                 |
| --------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `server-id`           | Plain text, e.g. `srv_<base64url>`                             | Stable per-`$BYSPACE_HOME` daemon ID. Overridable via `BYSPACE_SERVER_ID`.            |
