...
Logs, console output, and a reset button are not user feedback. Neither is a platform API unless it is verified on every supported platform: React Native Web's `Alert.alert()` is a no-op, so browser failures must use rendered app UI such as the shared alert component.
...
App-level Playwright browser E2E lives in `packages/app/e2e/*.spec.ts` and runs via `npm run test:e2e --workspace=@bytetrue/byspace-app` (separate from Vitest E2E). App Playwright specs that hit real providers use `*.real.spec.ts` and run through `npm run test:e2e:real --workspace=@bytetrue/byspace-app`; the default app E2E project ignores that suffix so CI does not need provider credentials.
...
- Windows CI runs `packages/server/src/terminal/terminal.test.ts` in its own Vitest invocation. Native `node-pty` tests need a fresh Windows worker because a fork reused by prior test files can exit unexpectedly while running them; keep the isolated invocation when changing server test orchestration.
