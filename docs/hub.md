...
The human CLI login and daemon relationship are separate identities. `byspace hub login [url]` stores a durable organization-scoped CLI credential keyed by normalized Hub origin under `BYSPACE_HOME`. Origin resolution uses explicit command input, `BYSPACE_HUB_URL`, active login, then `https://hub.byspace.cc.cd`. Connect uses exact-origin authority to request a one-time enrollment token, then passes only that token to the daemon. The daemon generates and persists its own relationship credential.
