...
Paseo is a personal, Web-based development environment built around agents. A hosted browser interface and local CLI control agents running in your own development environment.
...
Paseo is a client-server system. The daemon (Node.js) runs on your machine, manages agent processes, and streams output in real time over WebSocket to the Web app or CLI.
