...
BySpace is a next-generation development environment built around agents. One interface to run, monitor, and interact with coding agents across desktop, mobile, terminal, and web.
...
The public CLI is `byspace`. Packaged daemons use `127.0.0.1:6777`; repository development uses `127.0.0.1:6778`. Runtime state lives under `$BYSPACE_HOME` (`~/.byspace` by default), and public environment variables use the `BYSPACE_*` prefix.
...
BySpace is a client-server system. The daemon (Node.js) runs on your machine, manages agent processes, and streams output in real time over WebSocket. Clients connect to the daemon — locally or remotely.
