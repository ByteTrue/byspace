...
Desktop dev launches its desktop-managed daemon with `BYSPACE_NODE_ENV=development`,
...
`EXPO_PUBLIC_BYSPACE_DEV_BUILD_LABEL`. The expanded desktop sidebar shows it in
...
Linux produces the `byspace-desktop` launcher and desktop entry. macOS produces
`Applications/BySpace.app` plus the `byspace-desktop` launcher. Both use the nixpkgs
...
- The **server itself** (e.g. when launched by the desktop app or `npm run start`) defaults to `~/.byspace` (see `packages/server/src/server/byspace-home.ts`).
...
BySpace worktrees expose the native iOS dev app through the `ios-simulator` service in `byspace.json`. The service URL serves the simulator preview at `/.sim`, so the preview link is `${BYSPACE_URL}/.sim`.
...
- **An iOS Simulator runtime with at least one iPhone device type**. Recent Xcode versions may not bundle a runtime — add one via Xcode → Settings → Components (older Xcode: "Platforms"). The service targets `iPhone 16 Pro` by default (override with `BYSPACE_IOS_DEVICE_TYPE`) and falls back to any iPhone; it fails with `No iPhone simulator device type is installed` when none exist.
...
`BYSPACE_ELECTRON_REMOTE_DEBUGGING_PORT` when a QA workflow requires a validated,
fixed port.
...
```bash
npm run verify:electron-cdp --workspace=@bytetrue/byspace-desktop
```

The verifier reads the same `EXPO_PORT` and
`BYSPACE_ELECTRON_REMOTE_DEBUGGING_PORT` environment names as desktop dev. Set an
...
`BYSPACE_PORT=57928` alone starts Metro on 57928 but Electron still loads 8081.
...
```bash
BYSPACE_PROFILE_APP_URL=http://localhost:19010 \
  npm run profile:workspace-switching --workspace=@bytetrue/byspace-app
```
...
React commits on the same browser clock. Set `BYSPACE_PROFILE_WORKSPACE_DIGITS`,
`BYSPACE_PROFILE_WORKSPACE_LRU_SIZE`, or `BYSPACE_PROFILE_WARM_QUIET_MS` to change the shape. Set
`BYSPACE_PROFILE_DUMP_COMMITS=1` to include the nested component breakdown for every commit.
Set `BYSPACE_PROFILE_RETAINED_REPEATS=5` to repeat the retained-LRU burst for a less noisy sample.
Set `BYSPACE_PROFILE_CPU_PATH=/tmp/workspace-switch.cpuprofile` to run a separate retained-LRU
capture after the latency scenarios and write a raw CDP CPU profile without contaminating their
timings.
Set `BYSPACE_PROFILE_TRACE_PATH=/tmp/workspace-switch.trace.json` to run another separate retained-LRU
capture and write a Chromium Performance trace with User Timing marks and screenshots. Open the
trace in the Chrome DevTools Performance panel. Set `BYSPACE_PROFILE_TRACE_INVALIDATIONS=1` only for a
focused invalidation capture; React Native's generated stacks make that mode highly intrusive.
Set `BYSPACE_PROFILE_TRACE_FOCUS=1` to include focus targets, durations, and JavaScript call stacks in
...
```bash
npm run profile:explorer-toggle --workspace=@bytetrue/byspace-app
```

The harness verifies port `6768`, opens the BySpace workspace, creates and warms the Explorer pane,
...
`BYSPACE_PROFILE_TRACE_PATH=/tmp/explorer-toggle.trace.json` or
`BYSPACE_PROFILE_CPU_PATH=/tmp/explorer-toggle.cpuprofile` for separate Chromium captures. Override
the app URL, daemon port, workspace, or server with the corresponding `BYSPACE_PROFILE_*` variables.
...
```bash
BYSPACE_PROFILE_APP_URL=http://localhost:19010 \
  npm run profile:composer-typing --workspace=@bytetrue/byspace-app
```
...
Playwright dispatch lateness. Set `BYSPACE_PROFILE_TYPING_KEYS`, `BYSPACE_PROFILE_TYPING_CADENCE_MS`,
`BYSPACE_PROFILE_TYPING_REPEATS`, or `BYSPACE_PROFILE_WORKSPACE_DIGIT` to change the scenario. Optional
`BYSPACE_PROFILE_CPU_PATH` and `BYSPACE_PROFILE_TRACE_PATH` captures run separately after the latency
measurements so profiling overhead does not contaminate them.

Set `BYSPACE_PROFILE_TYPING_SCENARIO=height-growth` to alternate `Shift+Enter` and printable input.
...
the daemon-global Git process limits in `$BYSPACE_HOME/config.json`:
...
Reload the daemon with `byspace reload`. Environment-variable overrides still require a restart because
...
The desktop-managed daemon disables the bundled web UI by default (`BYSPACE_WEB_UI_ENABLED=false`) because the desktop app already ships the renderer as `app-dist`. Shipping the same assets again inside `@bytetrue/byspace-server` would duplicate the ~10.8 MiB install. Desktop packaging also excludes `node_modules/@bytetrue/byspace-server/dist/server/web-ui/**` from the packaged app.
