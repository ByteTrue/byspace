...
- **Bracketed paste is part of authoritative input-mode state.** Track `CSI ? 2004 h/l` alongside application cursor keys and replay it after every snapshot reset. If a snapshot restores cells but not mode 2004, xterm turns a multiline paste into separate submitted lines even though rendering and transport latency remain healthy.
- **Browser clipboard images cross the existing file-upload boundary.** The browser reads PNG/JPEG/GIF/WebP bytes, the client uploads them through `file.upload.request`, and xterm pastes the daemon-local path. Gate this with `server_info.features.terminalClipboardImage`; serialize repeated asynchronous image-paste operations, but keep ordinary terminal input immediate. On Windows, treat Pi's `Alt+V` as an image-aware alias only when the browser clipboard contains an image; forward the original chord when no image/API/callback exists, but surface clipboard read failures instead of silently forwarding. Never send base64 through the PTY or read the daemon machine's clipboard.
