...
The daemon resolves the current CLI through `BYSPACE_CLI` when its launcher supplies one, or through the npm package shim for standalone installs. Terminal setup exposes that resolved executable to hooks as `BYSPACE_HOOK_CLI`; desktop and other daemon launchers do not know about the hook-specific variable. The generated command falls back to bare `byspace` if the hook env is missing and no-ops outside BySpace terminals because the `BYSPACE_TERMINAL_ID` gate remains first. BySpace also prepends the resolved CLI directory to each terminal `PATH` as a secondary fallback. All other behavior lives in `byspace hooks`: read the env, map the event, POST activity, and no-op/fail-open when anything is missing or unavailable.
