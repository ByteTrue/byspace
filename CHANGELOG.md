...
- Client publication is fixed forward in 0.7.8 without moving the 0.7.6 tag or rebuilding it from later workflow code; iOS remains maintained source/prebuild/test-only and excluded from active CD.
...
- Client publication is fixed forward in 0.7.8 without moving the 0.7.5 tag or rebuilding it from later workflow code.
...
- Client publication is fixed forward in 0.7.8 without moving the 0.7.4 tag or rebuilding it from later workflow code.
...
- Client publication is fixed forward in 0.7.8 without moving the 0.7.3 tag or rebuilding it from later workflow code.
...
- Recovers an npm post-publish verification race by retrying the same immutable tag after the exact registry tarball became available; no package was republished and no incomplete client asset reached the GitHub Release. Client publication is ultimately fixed forward in 0.7.8.
