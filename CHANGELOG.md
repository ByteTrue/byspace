...
- Sidebar project rows no longer collapse. The hover chevron is gone, clicking a project row does nothing, and every workspace stays visible. Collapsing the Pinned section and the status groups is unchanged.
...
- Added manual provider session import and agent-guided project setup.
- Added safe Agent tools for refining workspace and unpublished branch names.
- Added terminal clipboard image upload and Pi-aware terminal agent activity reporting.
- Added Local, Remote, and Both branch source labels.
- Added workspace hover cards with exact Agent states and project-aware automatic Host badges.
- Added readable Host names to pairing offers.

### Improved

- Improved Direct and Relay terminal throughput, Windows responsiveness, retained renderers, and revision-based resume.
- Improved terminal selection and copy on compact web layouts.
- Improved Timeline pagination, reconnect catch-up, rewind arbitration, and synchronization state.
- Moved conversation collapse and return-to-bottom controls into active pane headers.
- Unified Stable and prerelease clients on `relay.byspace.cc.cd` while retaining separate Stable and Beta Web App channels.

### Fixed

- Fixed Windows ConPTY multiline paste when bracketed-paste mode is not propagated.
- Fixed pushed-branch status when a same-name origin branch exists without configured tracking.
- Fixed Workspace action visibility on compact and native layouts.
- Blocked plaintext non-loopback Direct connections from the hosted HTTPS App.
