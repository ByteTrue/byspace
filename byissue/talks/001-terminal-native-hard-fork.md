...
2. hard fork 并转 terminal-native：适配器维护量大降；代价是手机端 agent 对话退化成手机宽度的 xterm，权限批准、工具卡片、精确 steering 要重做或放弃，`byissue/spec/agent-conversation.md` 大半失效。
...
- `byissue/spec/agent-conversation.md` 整份漂移，Epic 关闭时退役。
...
三条路：在 byspace 里重做 orca 的 UI，一个人做不完；hard fork orca，冻结等于独自维护 180 万行不熟的代码，最危险；当 orca 用户，用 Tailscale 或 SSH 补远程。AI 推荐第三条并建议试用两周，同时说明当天写入 `byissue/` 的三份文件前提动摇，暂不再动。抛出的分叉：桌面与 web 远程访问走 Tailscale 或 SSH 能否接受。
...
- 上游同步与断开从 Epic 内提到根 `byissue/issues/017-…`（第 8 节撤回后该 Issue 更名并改号为 `019-x-sync-upstream-to-0-8-0-beta.md`）。理由是断开上游是项目级决定，不属于任何一个 Epic；取舍表、冲突解法和断开点原样带走。
...
- 已执行：本 Talk；PR #31 合入 `main`；`byissue/vision/index.md` 改回 UI 与 terminal 共存；`byissue/issues/019-x-sync-upstream-to-0-8-0-beta.md`（由原「选择性同步并断开」扩展而来）；`byissue/epics/003-o-orca-terminal-agent-experience/` 重定为「吸收 orca 的 terminal agent 体验」。
