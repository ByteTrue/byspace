...
2. 若已发布，走 `prepare` → `verify` → `submit` → `merge` 全流程，`LAST_UPSTREAM_SHA` 用本轮的目标 `4eab53e24`（`v0.8.0-beta.1`）。按已确认的分歧清单处理冲突：保留面见 `byissue/spec/` 各章节，尤其 `agent-conversation.md`（Pi turn 结算规则）与 `desktop-updates.md`（macOS 13 下限、DMG 交接顺序）。
3. 合并后关闭对应 Issue，按 close 规则毕业回写。
4. 这是与上游的最后一次同步，合并后可以在 `byissue/vision/index.md` 的"跨区域边界"里记一句"自此断开上游同步"，具体措辞留到那天再定。
