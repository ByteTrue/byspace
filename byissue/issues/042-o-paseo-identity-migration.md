...
- **B1. `@getpaseo/*` → `@byspace/*`**：945 处 import、631 文件，外加 7 个 `package.json`、`knip.json`、2 处 vitest alias、`package-lock.json`、`scripts/package-bytetrue-baseline.mjs:77-86` 的包名断言、`.github/workflows/npm-release.yml:111` 的 npmrc 行与 `:105` 的 `bin.paseo` 断言。
...
- **B3. 用户可见的 i18n 源文案**：231 处，9 种语言。改完后**删除** `packages/app/src/i18n/branding.ts` 的运行时改名层与 `i18n/i18next.ts:23-31` 的调用，以及 `branding.test.ts`。该层存在的唯一理由就是"上游文案 + 运行时改名"，源文案改掉后它成为空操作。
  ...
  注意该层只匹配大写 `Paseo` / `PASEO_`；`branding.test.ts` 第二个用例之所以通过，是因为 `@getpaseo/client`、`hub.paseo.sh`、`.paseo/workflows` 都是小写。删它没有兼容损失（前提已确认），但要注意别在删之前依赖它的行为。
...
- **B1**：`@getpaseo/*` → `@byspace/*`，682 个文件 + 7 个 `package.json` + knip/vitest/打包断言/CI npmrc；`package-lock.json` 重新生成；`node_modules/@byspace` 软链重建。
