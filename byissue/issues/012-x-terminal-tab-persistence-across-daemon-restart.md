...
**归属：** 独立 issue；相关真相 `byissue/spec/terminal.md`（快照与恢复）、`docs/data-model.md`（存储设施）。
...
- **回写位置**：`byissue/spec/terminal.md` 新增「重启恢复」节并补历史证据链接；`byissue/spec/index.md` Terminal 条目更新；`docs/data-model.md` 已随实现更新（目录布局 + 持久化语义）。
