...
BySpace 的原生推送**完全失效**：`packages/app/app.config.js` 的 `extra` 里没有 `eas.projectId`（fork 时被剥离，根因见 `byissue/issues/011-x-ff-android-apk-ci-release.md`），`push-notifications/internal/subscriptions.ts` 里 `getExpoPushTokenAsync` 拿不到 token，直接 warn 后放弃。
...
- `byissue/spec/connection.md` 已规定 HTTPS 页面拒绝非 loopback 的 `ws://`。所以走 HTTPS 的 PWA 只能经 Relay 或 `wss://` 连 daemon，**和局域网明文直连互斥**。原生 app 无此限制。
...
- 新增 `byissue/spec/notifications.md`——各平台走哪条、Web Push 为什么是这个形状（凭据归属、强制加密、一个 host 一个注册、手势要求）、以及五条长期约束。
- `byissue/spec/index.md` 加入口。
- `byissue/spec/connection.md` 补上「装成 PWA 与局域网明文直连互斥」及三条出路，并链向新 spec。
