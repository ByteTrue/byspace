...
- 回写候选：`byissue/spec/connection.md` 安全边界一节补充"局域网放开与密码的强制绑定"；若有坑点（如 listen 改写与 override 优先级的交互）记 note
...
**毕业回写：** `byissue/spec/connection.md` 安全边界一节新增"局域网监听以密码为前提"一条（不变量、哈希边界、重启生效、override 拒绝、能力门控）。
