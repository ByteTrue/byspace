...
`byissue/epics/004-x-worker-domain/spec.md` 已定形态与边界。本切片在动实现之前先把地基钉住，因为地基决定后面几块能不能少改动。
...
关闭时建议写入 `byissue/epics/004-x-worker-domain/spec.md`：
...
**目标达成。** 切片 001–013 全部完成并在真机上验证；关闭条件逐条有证据（见 epic spec 的核对表）。

**质量证据（关闭时复核，发现三项不成立并已补齐）：**

- 安全性：四类（实际六类）威胁规则各有拦截用例，并加了结构性测试保证新类别不会无覆盖入场；worker 无法自批。
- 资源效率：**关闭核对时发现预算只被报告、从未被强制**。已实现触发即拒绝唤醒（仍允许非唤醒消息），并补上**并发上界**（一个 worker 同时只跑一个任务）。
- 可用性：**关闭核对时发现窄屏下正文塌成每行一个字符**（我原先的溢出检查通过，因为没有溢出）。已修，并发现组报告复用了单行样式导致横向裁切。
- 可维护性：8 角色 / 47 技能全部为数据。

**毕业回写：** `byissue/spec/worker.md`（+ `byissue/spec/index.md` 体验地图条目）。只写了代码重建不出来的部分：词汇、长期边界与取舍、被排除方案、明确不属于的范围。实现细节刻意留给代码。

**沉淀：** `byissue/notes/003-checks-that-pass-without-testing.md`（九种"绿勾但没测到东西"）。

**遗留事项（不在本 Epic 关闭范围内，登记于 spec 的「已知不足」）：** 名册屏幕的反应性、逐工具裁决接入执行路径、记忆、技能接入运行时。前三者都有明确的升级触发条件。
