...
| 条件                                         | 证据                                                                                                                                                                                                    | 状态 |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 第一批五项完成并验证                         | 切片 001–013；e2e verifier 34 项对真实 daemon                                                                                                                                                           | ✅   |
| 安全性：四类规则各有拦截用例                 | `tool-guard.test.ts` 覆盖全部**六**个类别（含新增的 `sensitive_file_access` 与 `network_abuse`），并加了一条"每个类别都必须有样例命令"的结构性测试 —— 原来 `sensitive_file_access` 有规则却**没有测试** | ✅   |
| 安全性：worker 无法自批                      | 切片 001 的 `assertNotSelfPermissionApproval`                                                                                                                                                           | ✅   |
| 资源效率：并发与预算有上界，超限停止自动唤醒 | 切片 012 的预算 + 本次补的**并发上界**（一个 worker 同时只跑一个任务）与**触顶停止唤醒**（仍允许非唤醒消息）                                                                                            | ✅   |
| 可靠性：状态转换单一入口                     | 九态单一写入器 + 穷举负例测试                                                                                                                                                                           | ✅   |
| 可靠性：重启后目标/消息/任务不丢             | 迁移在真实库上验证（v4→v5 保留数据）；reopen 重置预算                                                                                                                                                   | ✅   |
| 可用性：窄屏可读可操作                       | 390×844 下核对：**发现并修掉两个真缺陷**（见下）                                                                                                                                                        | ✅   |
| 可维护性：接入角色成本 = 一次模板导入        | 8 角色 / 47 技能全部为数据（目录即目录清单）                                                                                                                                                            | ✅   |
| 不侵入既有 spec                              | 未改动 `agent-conversation.md` / `terminal.md` / `workspace.md`                                                                                                                                         | ✅   |

**窄屏核对抓出的两个真缺陷（这正是「必须真机验证」的价值）：**

1. **独立 console 完全没有窄屏处理** —— 侧栏固定 264px，390px 下正文只剩约 126px，渲染成**每行一个字符**。我原先的程序化检查（`overflowX`）**通过了**，因为没有溢出：布局只是塌掉了。**溢出检查不等于可用性检查。** 已按仓库既有约定（`useIsCompactFormFactor`）改成窄屏下侧栏变成顶部横条。
2. **组报告复用了旧的单行样式**（`styles.row` 是 `flexDirection: "row"`），导致报告各段横向排列并被裁掉。已改成纵向卡片，并删掉因此失效的两个样式。
