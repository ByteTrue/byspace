...
**只做吸收。** 不迁移、不替换、不删除。结构化会话与 terminal 会话是两条并行的一等路径，见 `byissue/vision/index.md`「产品核心」。

- 来源 Vision：`byissue/vision/index.md`「参考产品」与「探索中或候选」。
...
- `byissue/spec/terminal.md` 的字符与顺序语义、daemon 重启后的标签恢复。
...
**不在本 Epic：** 上游同步，见 `byissue/issues/019-x-sync-upstream-to-0-8-0-beta.md`。建议同步完成后再开始实现，避免吸收改动与全量同步的大 diff 互相冲突。
...
- `byissue/talks/001-terminal-native-hard-fork.md` — 为什么两条路共存、为什么不以 orca 为底、撤回的完整经过。
