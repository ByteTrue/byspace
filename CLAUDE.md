...
- **For any upstream source-project update request, use the repo-local `upstream-sync` skill.** `.byspace/upstream-sync.json` is the cursor; never merge upstream wholesale, import tags, omit commits from the user ledger, or replay before approval.
