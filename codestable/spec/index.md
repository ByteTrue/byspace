...
- Go daemon 已按 Cloudflare Worker Relay v2 的 `connected` / `disconnected` / `sync` control contract 打开 outbound data sessions，以 stable private identity 和 v3 offer 完成 fresh-challenge HMAC client authentication，并与 direct `/ws` 共用 Agent dispatcher。Copied client 跨语言 E2E 已验证 remote create/send/live Timeline、Relay/daemon restart、Pi resume、zero-plaintext wire、replay/tamper/wrong-token fail closed；GitHub Actions run 33170940010 已从 commit `6d939b4` authenticated deploy Worker version `b20690a6-41c1-47f2-80e9-b5327a51d939`，并通过 production HTTP boundaries、raw bridge 与 full daemon/client/CLI smoke。
