...
Derived from: [Terminal 原始性能基线与差距定位](./025-o-terminal-direct-baseline/index.md)
...
- BySpace 自 v0.1.0 已有 `file.upload.request` 二进制上传、daemon 临时文件写入和 Relay 传输能力，不需要新增文件上传 RPC 或 Terminal 专用 capability。
...
5. Direct 和 Relay 共用相同语义与现有加密上传链路；不得增加仅 Direct 可用的旁路。
6. 不新增上传 RPC、不把图片渲染到 Terminal、不把 base64 文本送入 PTY。
7. 浏览器级测试覆盖图片剪贴板到 daemon 文件路径输入，验证上传后的 bytes；runtime 测试覆盖图片优先、无图片时文本 paste、图片路径 paste、Windows `Alt+V` 图片分支及无图片 fallback。
...
- `packages/app/src/components/terminal-pane.tsx`
  - 调用自 v0.1.0 已存在的 `client.uploadFile()`；显示错误 toast。
- Terminal runtime browser test 与隔离 daemon E2E。
...
- TerminalPane 直接复用自 v0.1.0 已存在的 `client.uploadFile()`；图片 paste 是纯 client 行为，不增加 Terminal 专用 daemon capability 或旧-host gate。runtime 成功取得 daemon path 后不依赖 xterm live mode，始终强制作为一个 bracketed block 输入。
- 未新增 RPC、binary opcode 或 Direct 旁路。
...
- `npm --workspace @bytetrue/byspace-app run test:browser -- src/terminal/runtime/terminal-emulator-runtime.browser.test.ts --bail=1`：31/31 通过；覆盖标准快捷键不被 keydown 接管、同一事件 text+image 统一上传图片并只粘贴 daemon path、upload rejection 后队列继续，以及 unmount 后无 stale error。
...
- `npx vitest run packages/protocol/src/terminal-input-mode.test.ts packages/protocol/src/messages.terminal-restore.test.ts --bail=1`：通过；restore schema 兼容旧消息。
...
- 关闭证据包括 Browser runtime 31/31、Terminal E2E 5 个场景，以及 typecheck、lint、format、Web export、diff check 全部通过。
