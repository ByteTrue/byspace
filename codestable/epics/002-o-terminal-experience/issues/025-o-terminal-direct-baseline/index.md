...
对应改变已关闭：`026-x-terminal-bracketed-paste-restore` 在 snapshot/reload 后恢复 mode 2004；`027-x-terminal-clipboard-image-paste.md` 复用现有 binary file upload，把浏览器图片写到 daemon 并 paste 服务端路径。用户已在 macOS Chrome + PixPin + Direct headed 验收同一 paste 同时携带文本 path 与图片时统一走 daemon upload。Browser runtime 33/33、Terminal E2E 5/5 均通过。
...
- 稳定 How it works 结论：用户确认关闭后进入 `codestable/spec/index.md` 的 Terminal 当前真相；除 Direct 性能基线外，还需毕业输入模式恢复与浏览器/daemon 剪贴板责任边界。
- 需要更新的阅读入口：`codestable/spec/index.md` 增加 Terminal 主路径与 Direct/Relay 质量分层。
...
- 更新的 spec index：`codestable/spec/index.md`。
