...
- 隶属 epic：`codestable/epics/002-o-terminal-experience/spec.md`
- 前序 issue：`codestable/epics/002-o-terminal-experience/issues/006-x-terminal-retained-panel-layout.md`
...
- epic：`codestable/epics/002-o-terminal-experience/spec.md` 已记 retained emulator 不再随 workspace 失焦卸载，并链入后续的 014。
...
- 遗留：历史仍受 daemon 1000 行保有量与“每次切回重传”双重制约 → `codestable/epics/002-o-terminal-experience/issues/014-x-terminal-restore-resume-from-revision.md`。
...
代价是每次切回多约 90ms 的重传（该用例每行很短，回放文本约 10KB；真实内容会大得多，Relay 下还要加 RTT 与加密）。这是用传输成本换历史长度的权宜，天花板仍在；根治方案是 `codestable/epics/002-o-terminal-experience/issues/014-x-terminal-restore-resume-from-revision.md`（按 revision 续传，切回不重传也不截断），014 落地后可以把窗口调回小值。
