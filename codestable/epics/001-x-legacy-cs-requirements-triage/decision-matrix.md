...
| ID  | reset 前证据                                                                    | 当前 `main` 证据                                                                                                                 | 证据状态             | Owner 决定                         |
| --- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ---------------------------------- |
| B01 | 归档 closed；旧 BySpace 根域、stable/beta App 和 stable/beta Relay 迁移曾完成。 | Owner 于 2026-09-02 取消 Beta Relay；Stable 与 prerelease 统一使用 `relay.byspace.cc.cd:443`，仅 Web App 保留 Stable/Beta 路由。 | **Owner 决策已更新** | **保留双 App；Relay 收敛为单通道** |
...
B01 最终决策：Beta Relay 曾经部署并可用，但 Owner 于 2026-09-02 明确取消该分流。Stable 与 prerelease 统一使用 `relay.byspace.cc.cd:443`；`relay-beta.byspace.cc.cd` 和 Worker `byspace-relay-beta` 删除。
