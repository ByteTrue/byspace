...
- 该 push run 现在符合 Publisher/App Deploy 的 exact-SHA 成功 CI 选择条件，但没有实际 release tag；因此不能把 CI 证明冒充 npm registry、Pages 或渠道 tuple 证明。Issue 保持 open，等待下一次真实 Beta 与下一次真实 Stable 发布。
