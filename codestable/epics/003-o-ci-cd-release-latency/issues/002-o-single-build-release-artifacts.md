...
- exact-SHA `6044abc83` 的 push CI run `31495966222` 首次让完整 12-shard CI 与三平台 distribution 全绿。`package-artifact` 从 workflow 起点并行生成 Web SHA-256 `7cd4a54e…d585f` 与 npm SHA-256 `398f5f1f…b13d`；Ubuntu、macOS、Windows 均回验并 smoke 同一 npm digest，Windows readiness 约 1 秒完成。随后同 SHA 的 4 个 workflow-dispatch run 也全部通过三平台 artifact 链。
- 该 push run 符合 Publisher/App Deploy 的 exact-SHA 成功 CI 选择条件，但当时没有实际 release tag，因此没有把 CI 证明冒充 npm registry、Pages 或渠道 tuple 证明；真实 Beta 证据由下述发布补齐。
- 真实 Beta `v0.5.0-beta.1` 已从 release commit `c4789107a` 发布：exact-SHA push CI run `31517745431` 27/27 jobs 全绿，`Publish npm` run `31521634441` 直接晋升 CI tarball，registry 回下载与 CI artifact 逐字节一致（SHA-256 `33b70965…54db3`）；`Deploy App` run `31521710256` 将同一 CI Web artifact 部署到 Beta Pages source `c478910`，`Deploy Relay` run `31521710212` 部署 `v0.5.0-beta.1 (beta)`。
- 发布 workflow runner 执行窗口从 Publisher job start 到最后 App `Complete job` 为 1:59；GitHub workflow created/updated envelope 为 2:05，外部调度与收尾 6 秒单独报告。npm `latest` 仍为 `0.4.0`，Stable Pages deployment/source 与 Stable Relay deployment/version 均逐项保持不变。
- 从 registry `@beta` 干净全局安装的 daemon 报告 `0.5.0-beta.1`，生成 Beta App/Relay pairing offer；真实浏览器通过 `app-beta` → `relay-beta` 显示 Online / Relay / `v0.5.0-beta.1` 并稳定 30 秒，随后 lifecycle RPC 正常停机。该验收同时修正了 `prove-relay-prod.mjs` 已漂移的旧 Settings card 路由和缺失 TLS 标记。
- Beta tuple 已有真实证据；Issue 继续保持 open，只等待下一次真实 Stable 发布验证 Stable tuple、Beta 不变量与同一 artifact promotion 路径，不为验收额外制造 Stable 版本。
