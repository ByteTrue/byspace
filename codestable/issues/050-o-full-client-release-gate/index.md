---
kind: issue
title: 完整客户端发行闸门与 v0.7.7 恢复发布
type: bug
status: open
created: 2026-08-27
---

# 完整客户端发行闸门与 v0.7.7 恢复发布
...
- **兼容性：** `v0.7.0` 至 `v0.7.6` 的 tag/资产均不移动。`v0.7.1` 至 `v0.7.6` 的客户端矩阵都在资产上传前失败；通过 `v0.7.7` fix-forward 完成全量发行。Stable/Beta Web、Relay、npm 与 Desktop update channel 继续隔离。
...
`v0.7.6` 的 npm、Stable Web 与 Stable Relay 已成功，macOS arm64/x64 与 Linux x64 完整客户端 job 也已成功，但矩阵仍在 aggregate upload 前停止：Windows 的 `@electron/asar` API 返回反斜杠路径，verifier 未做平台无关归一化；Android 首次 hosted runner 在 Gradle release build 期间取消操作。诊断性 failed-job rerun 随后成功构建并以固定 v1 证书签名 APK，但暴露出只识别旧 `Signer #1` 标签的证书摘要解析器，尚未进入包名/版本和 emulator 验证。GitHub Release 客户端资产仍为 0。
