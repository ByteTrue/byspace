---
kind: issue
title: 完整客户端发行闸门与 v0.7.2 恢复发布
type: bug
status: open
created: 2026-08-27
---

# 完整客户端发行闸门与 v0.7.2 恢复发布
...
- **兼容性：** `v0.7.0` 与 `v0.7.1` tag/资产均不移动。`v0.7.1` 已发布 npm/Web/Relay，但首次客户端矩阵在资产上传前失败；通过 `v0.7.2` fix-forward 完成全量发行。Stable/Beta Web、Relay、npm 与 Desktop update channel 继续隔离。
...
6. 本地完成三平台/Android 可执行验证与完整 release gate；`v0.7.1` 首次客户端矩阵暴露 CI 机械缺陷后，不修改旧 tag，修复并切 `v0.7.2`，等待 npm/Web/Relay/Clients 全部完成并做真实安装验证。
...
- CI 穿刺：workflow 静态契约、本机 macOS/Android 产物验证与 exact-SHA 主 CI 先通过；`v0.7.2` tag 端到端执行 macOS/Linux/Windows/Android publisher。客户端工作流不提供绕过上传的假发布模式。
- 发布：`v0.7.2` exact-SHA CI；npm latest、Stable Web、Stable Relay、Publish Clients 全绿。
