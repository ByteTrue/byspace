---
kind: issue
title: 完整客户端发行闸门与 v0.7.1 补发
type: bug
status: open
created: 2026-08-27
---

# 完整客户端发行闸门与 v0.7.1 补发
...
- upstream `desktop-release.yml` 会在 release tag 构建并上传三平台产物；BySpace 当前只允许 manual dispatch，且默认不 publish。
- upstream `android-apk-release.yml` 会在 tag 构建并上传 APK；BySpace 当前只生成 Actions artifact，不写入 GitHub Release。
- BySpace 的 iOS EAS workflows 仍处于可手动触发目录；这不符合用户再次确认的“只保留、CD 不发布 iOS”边界。
...
- **可维护性：** Stable/Beta、manual recovery 与文档读取同一 release tuple；静态测试阻止恢复 `checkout_ref`、`--clobber`、debug-signed Android、active iOS publish 或 Web-only 完成声明。
- **兼容性：** `v0.7.0` tag/资产不移动；通过 `v0.7.1` 修复发行。Stable/Beta Web、Relay、npm 与 Desktop update channel 继续隔离。

## 实现安排

1. 把 Desktop 与 Android 合并为一个 `Publish clients` tag workflow：tag/manual retry 都先校验 immutable tag、当前 `origin/main` 与 exact-SHA push CI。
...
6. 本地完成三平台/Android 可执行验证与完整 release gate；推送 exact-SHA CI 全绿后切 `v0.7.1`，等待 npm/Web/Relay/Clients 全部完成并做真实安装验证。
...
- CI 穿刺：workflow 静态契约、本机 macOS/Android 产物验证与 exact-SHA 主 CI 先通过；`v0.7.1` tag 首次端到端执行 macOS/Linux/Windows/Android publisher。客户端工作流不提供绕过上传的假发布模式。
- 发布：`v0.7.1` exact-SHA CI；npm latest、Stable Web、Stable Relay、Publish Clients 全绿。
