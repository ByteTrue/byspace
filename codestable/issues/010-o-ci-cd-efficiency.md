...
- 2026-09-05：Android APK 发布 CI 化（ff 011）：发布七件套最后的手工环节消除；顺带修复 secrets 里 8/26 旧 keystore 未随 8/29 本地 v2 更新的漂移（产物对账拦截）。
- 2026-09-04（晚）：四阶段全部处置完毕——阶段 3 改判不建（上游无此机制、根因已灭、契约冲突）；阶段 4 评估不做（shard 4→6 代价大于 ~5min 收益）；windows observation flake 真根因修复（fake timer 冻结在真实时间 waitFor 内，改为每轮轮询推进 fake 时间，`49795a571`）；browser capture 定位为 linux 无头合成器表面丢失（上游同款 harness 同有，挂起独立调查）；launch-memory 降级观察。剩余唯一未闭环项：真实发布验证（见「关闭时」）。
